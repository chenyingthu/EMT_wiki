...
Due to the nonlinear nature of the corona effect, most of the approaches adopted to represent this phenomenon in wave propagation calculations are based on numerical solutions, such
...
as the finite difference time domain (FDTD) [4], [5], finite element method [6], [7], or method of characteristics [8]. However, these approximations are not directly applicable to EMT-type programs since the line terminals cannot be represented by Norton’s equivalents.
...
![](images/ac3c4d2a7084dd604b0779d187e26146975336a0cab4c717be56c6b8cf382fad.jpg)  
Fig. 1. Incident and reflected current waves at transmission line ends k and m.

represent the corona effect in EMT-type platforms. At the current stage, the VFDLM can only deal with single-phase transmission lines.
...
$$
- \frac {d V}{d x} = Z (s) \cdot I \tag {1}
$$

$$
- \frac {d I}{d x} = Y (s) \cdot V \tag {2}
$$
...
![](images/ff9b3c57e87e3e27b80db2e9fba457d59afc43e617b22b6660d1b470bbe136db.jpg)  
...
$$
I _ {k i} = H \cdot I _ {m r} \tag {6}
$$

$$
Y _ {c} = Z ^ {- 1} \cdot \sqrt {Z \cdot Y} \tag {7}
$$

$$
H = e ^ {- \sqrt {Y \cdot Z} l} \tag {8}
$$
...
$$
i _ {k} (t) = G _ {s} \cdot v _ {k} (t) - i _ {\text {h i s t k}} (t) \tag {9}
$$

$$
i _ {k r} (t) = i _ {k} (t) + i _ {k i} (t) \tag {10}
$$

$$
i _ {k i} (t + \Delta t) = H * i _ {m r} (t - \tau) \tag {11}
$$

$$
i _ {h i s t, k} (t + \Delta t) = Y _ {c} * v _ {k} (t) - 2 i _ {k i} (t + \Delta t) \tag {12}
$$
...
$$
Y _ {c} \cong d + \sum_ {i = 1} ^ {N _ {p} Y _ {c}} \frac {r _ {i} ^ {Y _ {c}}}{s - p _ {i} ^ {Y _ {c}}} \tag {13}
$$

$$
H \cong \sum_ {i = 1} ^ {N _ {p} H} \left(\frac {r _ {i} ^ {H}}{s - p _ {i} ^ {H}}\right) \cdot e ^ {- s \tau} \tag {14}
$$
...
$$
a (t) = \alpha \cdot a (t - \Delta t) + r \left\{\lambda \cdot c (t) + \mu \cdot c (t - \Delta t) \right\} \tag {16}
$$
...
Applying the recursive convolution to solve (11) and (12) and $\mathrm { i f } Y _ { c }$ and H were fitted as in (13) and (14), after some algebraic manipulations, we can obtain the set of equations shown in (20) $- \left( 2 2 \right)$ , which characterizes the implementation of the WB line model in EMT-type programs.

$$
G _ {s} = d + \sum_ {i = 1} ^ {N _ {p} Y _ {c}} r _ {i} ^ {Y _ {c}} \cdot \lambda_ {i} ^ {Y _ {c}} \tag {20}
$$

$$
\begin{array}{l} y _ {c} ^ {\prime} * v _ {k} = a _ {Y} (t) = \sum_ {i = 1} ^ {N _ {p} Y _ {c}} \left(\alpha_ {i} ^ {Y _ {c}} \cdot a _ {Y} (t - \Delta t) \right. \\ \left. + r _ {i} ^ {Y _ {c}} \cdot \mu_ {i} ^ {Y _ {c}} \cdot v _ {k} (t - \Delta t)\right) \tag {21} \\ \end{array}
$$
...
$$
Y (s, v) = G (v) + s C (v) \tag {25}
$$

With this modification, the characteristic admittance and propagation function should be rewritten as:

$$
Y _ {c} (s, v) = Z ^ {- 1} (s) \cdot \sqrt {Z (s) \cdot Y (s , v)} \tag {26}
$$

$$
H (s, v) = e ^ {- \sqrt {Y (s , v) \cdot Z (s)} l} \tag {27}
$$
...
![](images/5dfae1bcbc743aeee5953086ac295aacb2b2501d1ac16b53e308045503f683d2.jpg)  
(a)

![](images/b8d95079f2c26ce105cbf08afcea8a10a9178bd718c1c20e967264057882b96a.jpg)

![](images/0910b861040d04dc38045ef729777f3a6678a7a1c62b764801266dc0b548f597.jpg)  
（c)

![](images/bed25e773439cefb7bd0eacd336e279483788ec919baa99799e25838f535a232.jpg)  
(d)   
...
![](images/8c283c06631df953e24adc59f6a67788317030fbc239787c51b9ffc2917b17f9.jpg)

![](images/7732074924fc3a59ef9798edf612f0ce89a4dd086db961c7dc92e661ad61c9d0.jpg)  
...
![](images/422670f1bf2447431e3d69d9ad73edcc4c972de62bd0f75befa012af6a90158f.jpg)  
...
$$
B S = \left\lfloor \frac {\tau (v _ {m a x})}{\Delta t} \right\rfloor + 1 \tag {28}
$$
...
$$
\left[ \boldsymbol {Y} _ {\mathbf {s h}} \right] \cdot \left[ \boldsymbol {v} (t) \right] = \left[ \boldsymbol {i} (t) \right] - \left[ \boldsymbol {i} _ {\text {h i s t}} (t) \right] \tag {29}
$$
...
![](images/8a41efab54f823dbed1ea56f12391a8de1e6ac416e9064b1378c10ee7ff83708.jpg)  
(@)

![](images/4d3c2d70d9daaf39b048ec6e75f31c3be81d443b60ae0ce2a4e27408f5336ebb.jpg)  
(b)

![](images/eed8ccb42139e3bb098be6f8d5cc940a7aece3b46938741870e363bcc4cc0200.jpg)  
（c）  
...
TABLE II CORONA MODEL PARAMETERS   

<table><tr><td>Line</td><td>σC(F/m)</td><td>σG(S/m)</td><td>vcrit(kV)</td></tr><tr><td>Tidd</td><td>14.5</td><td>4·106</td><td>470</td></tr><tr><td>EDF</td><td>8.5</td><td>4.8·106</td><td>250</td></tr><tr><td>Shiobara</td><td>13.5</td><td>4·106</td><td>303</td></tr></table>

![](images/42603b069e6e9f1549cb582fcd724e065cdf85d61d28fa398558b7d7b33855c2.jpg)  
Fig. 9. Influence of voltage sampling in VFDLM.

TABLE IIITRANSMISSION LINES PARAMETERS  

<table><tr><td>Line</td><td>Length (km)</td><td>Height (m)</td><td>Conductor radius (mm)</td><td>Soil resistivity (Ω·m)</td><td>Matching resistance (Ω)</td></tr><tr><td>Tidd</td><td>4</td><td>18.89</td><td>25.40</td><td>100</td><td>470</td></tr><tr><td>EDF</td><td>12</td><td>12</td><td>13.2</td><td>100</td><td>459</td></tr><tr><td>Shiobara</td><td>3</td><td>16.87</td><td>12.65</td><td>100</td><td>486</td></tr></table>

![](images/f51a578933f0f25467dc56fa32721dcf03b8bc866f6717b2047ce81e3a968474.jpg)  
Fig. 10. Structure adopted to reproduce the field measurement tests in the simulations.

3. The ideal number of voltage samples for which the TDIP should be calculated evidently varies according to the voltage range. In the case of the Tidd line, for instance, Fig. 9 presents the results for $n = 2 0 , n = 5 0 , n = 1 0 0$ and n = 200 voltage samples linearly distributed inside the range $4 7 0 \leq v \leq 2 0 0 0 \mathrm { k V } .$ All the cases present reasonable results in terms of accuracy. However, as shown in the zoomed portion, spurious oscillations can be observed for the cases with n = 20 and n = 50. These spurious oscillations occur due to the abrupt variation in the TDIP from one integration step to the other. By increasing the number of voltage samples to $n = 1 0 0 \mathrm { o r } n = 2 0 0$ , we have a smooth variation along the voltage range, leading

to smooth results. Furthermore, as seen in the zoomed portion, no significant differences are observed when increasing the number of voltage samples from n = 100 to $n \ = \ 2 0 0$ , which explains the adoption of n = 100 for all analyzed cases in Section IV.
...
$$
C (v) = \left\{ \begin{array}{l} C _ {0}, \\ C _ {0} + \sigma_ {C} \sqrt {\frac {r}{2 h}} \cdot \left(1 - \frac {v _ {c r i t}}{v}\right) \cdot 2 0 ^ {- 1 1}, v > v _ {c r i t} \end{array} \right. \tag {31}
$$

$$
G (v) = \left\{ \begin{array}{l l} G _ {0}, & v \leq v _ {\text {c r i t}} \\ G _ {0} + \sigma_ {G} \sqrt {\frac {r}{2 h}} \cdot 1 0 ^ {- 1 1} \left(1 - \frac {v _ {\text {c r i t}}}{v}\right) ^ {2} & v > v _ {\text {c r i t}} \end{array} \right. \tag {32}
$$
...
![](images/be0719140c7826c101c6e4470d9eb27a545610e7053200dcc091564e4c3e7383.jpg)
...
![](images/47350de928a48a6d90b3a9d2fb9d3ab6d829db8fafe882a3e4d09e5ac8db7ff6.jpg)
