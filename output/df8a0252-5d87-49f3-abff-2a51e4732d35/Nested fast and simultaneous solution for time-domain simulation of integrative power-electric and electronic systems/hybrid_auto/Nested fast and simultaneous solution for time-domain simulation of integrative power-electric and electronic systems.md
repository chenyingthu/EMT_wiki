...
AN important field of application for the simulators ofthe Electromagnetic Transients Program (EMTP) type the Electromagnetic Transients Program (EMTP) type [1] is the time-domain study of integrative power-electric and electronic systems. Examples of timely interest include simulations of storage and compensation systems [2], [3]; distributed generation (DG) [4], [5]; all-electric ship power systems; and high-voltage direct current (HVdc) systems [6]–[8]. The interfacing of the power-electronic converters and networks can be made through disconnection and prediction of the common

Manuscript received March 7, 2005; revised September 30, 2005. This work was supported by the National Science Foundation under Award ECS-0238523. The authors are with the Department of Electrical Engineering, SESAME Laboratory, University of Washington, Seattle, WA 98195-2500 USA (e-mail: strunz@ee.washington.edu).

Digital Object Identifier 10.1109/TPWRD.2006.876657
...
$$
\left( \begin{array}{c c} \boldsymbol {A} _ {1 1} & \boldsymbol {A} _ {1 2} \\ \boldsymbol {A} _ {2 1} & \boldsymbol {A} _ {2 2} \end{array} \right) \left( \begin{array}{c} \boldsymbol {\xi} _ {1} \\ \boldsymbol {\xi} _ {2} \end{array} \right) = \left( \begin{array}{c} \boldsymbol {b} _ {1} \\ \boldsymbol {b} _ {2} \end{array} \right) \qquad \boldsymbol {A} _ {1 1} = \left( \begin{array}{c c c} \boldsymbol {A} _ {1 1 1} ^ {\text {c c c}} & & \\ & \ddots & \\ & & \boldsymbol {A} _ {1 1 M ^ {\text {c c c}}} ^ {\text {c c c}} \end{array} \right)
$$
...
$$
\boldsymbol {A} ^ {\cap} = \boldsymbol {A} _ {2 2} - \boldsymbol {A} _ {2 1} \boldsymbol {A} _ {1 1} ^ {- 1} \boldsymbol {A} _ {1 2} \tag {1}
$$

$$
\boldsymbol {b} ^ {\cap} = \boldsymbol {b} _ {2} - \boldsymbol {A} _ {2 1} \boldsymbol {A} _ {1 1} ^ {- 1} \boldsymbol {b} _ {1}. \tag {2}
$$
...
![](images/6b935d07938aefe3a913915d8ae7f6fc48a2960d9563e7eb8aa73cd8e726c2eb.jpg)  
...
$$
\boldsymbol {Y} _ {\mathrm {B}} \boldsymbol {v} _ {\mathrm {B}} = \boldsymbol {j} _ {\mathrm {B}} + \boldsymbol {i} _ {\mathrm {B}} \tag {3}
$$
...
$$
\left( \begin{array}{l l} \boldsymbol {Y} _ {\mathrm {d d}} & \boldsymbol {Y} _ {\mathrm {d e}} \\ \boldsymbol {Y} _ {\mathrm {e d}} & \boldsymbol {Y} _ {\mathrm {e e}} \end{array} \right) \left( \begin{array}{l} \boldsymbol {v} _ {\mathrm {d}} \\ \boldsymbol {v} _ {\mathrm {e}} \end{array} \right) = \left( \begin{array}{c} \boldsymbol {j} _ {\mathrm {d}} \\ \boldsymbol {j} _ {\mathrm {e}} + \boldsymbol {i} _ {\mathrm {e}} \end{array} \right) \tag {4}
$$

where $j _ { \mathrm { d } } , j _ { \mathrm { e } }$ are, respectively, the source-dependent nodal current injections into the nodes with unknown and known voltages

and $\dot { \pmb { \imath } } _ { \mathrm { e } }$ are the currents that flow through the nodes with known voltages.
...
$$
\boldsymbol {L} _ {\mathrm {d d}} \boldsymbol {U} _ {\mathrm {d d}} \boldsymbol {v} _ {\mathrm {d}} = \boldsymbol {j} _ {\mathrm {d}} - \boldsymbol {Y} _ {\mathrm {d e}} \boldsymbol {v} _ {\mathrm {e}}. \tag {5}
$$
...
$$
\boldsymbol {Y} _ {\mathrm {N}} ^ {\subset} \boldsymbol {v} _ {\mathrm {t}} ^ {\subset} = \boldsymbol {j} _ {\mathrm {N}} ^ {\subset} + \boldsymbol {i} _ {\mathrm {t}} ^ {\subset} \tag {6}
$$
...
![](images/e4e67145e7eea41339b0fa4266e723b499681eaf06de02e9d4be28b238dcbb67.jpg)  
Fig. 3. AC network on the inverter side of the CIGRE HVdc benchmark model represented as a child subdivision.

Step 1: Calculate nodal voltages: ${ \pmb Y } _ { \mathrm { d d } } ^ { \cap } { \pmb v } _ { \mathrm { d } } ^ { \cap } = { \pmb j } _ { \mathrm { d } } ^ { \cap } - { \pmb Y } _ { \mathrm { d e } } ^ { \cap } { \pmb v } _ { \mathrm { e } } ^ { \cap } .$   
Step 2:For all child subdivisions do: perform child simulations.   
Step 3:If there is a status change of the network,then do: Step 3.1: Reconstruct $\mathbf { Y } _ { \mathrm { d d } } ^ { \mathrm { n } }$ accordingly.

Step 32:Refactoize $\mathbf { \Delta } \mathbf { Y } _ { \mathrm { d d } } ^ { \cap \cdots }$ into $\pmb { L } _ { \mathrm { d d } } ^ { \cap }$ and $U _ { \mathrm { { d d } } } ^ { \cap } .$
...
![](images/e083695ae4aba0be2adffcc6ed9b48579ce25b55b6e95817cec1b60adbc8a508.jpg)  
...
$$
\left( \begin{array}{c c c} \boldsymbol {Y} _ {\mathrm {d d}} ^ {\subset} & \boldsymbol {Y} _ {\mathrm {d e}} ^ {\subset} & \boldsymbol {Y} _ {\mathrm {d t}} ^ {\subset} \\ \boldsymbol {Y} _ {\mathrm {e d}} ^ {\subset} & \boldsymbol {Y} _ {\mathrm {e e}} ^ {\subset} & \boldsymbol {Y} _ {\mathrm {e t}} ^ {\subset} \\ \boldsymbol {Y} _ {\mathrm {t d}} ^ {\subset} & \boldsymbol {Y} _ {\mathrm {t e}} ^ {\subset} & \boldsymbol {Y} _ {\mathrm {t t}} ^ {\subset} \end{array} \right) \left( \begin{array}{c} \boldsymbol {v} _ {\mathrm {d}} ^ {\subset} \\ \boldsymbol {v} _ {\mathrm {e}} ^ {\subset} \\ \boldsymbol {v} _ {\mathrm {t}} ^ {\subset} \end{array} \right) = \left( \begin{array}{c} \boldsymbol {j} _ {\mathrm {d}} ^ {\subset} \\ \boldsymbol {j} _ {\mathrm {e}} ^ {\subset} + \boldsymbol {i} _ {\mathrm {e}} ^ {\subset} \\ \boldsymbol {j} _ {\mathrm {t}} ^ {\subset} + \boldsymbol {i} _ {\mathrm {t}} ^ {\subset} \end{array} \right). \tag {7}
$$
...
$$
\boldsymbol {L} _ {\mathrm {d d}} ^ {\subset} \boldsymbol {U} _ {\mathrm {d d}} ^ {\subset} \boldsymbol {v} _ {\mathrm {d}} ^ {\subset} = \boldsymbol {j} _ {\mathrm {d}} ^ {\subset} - \boldsymbol {Y} _ {\mathrm {d e}} ^ {\subset} \boldsymbol {v} _ {\mathrm {e}} ^ {\subset} - \boldsymbol {Y} _ {\mathrm {d t}} ^ {\subset} \boldsymbol {v} _ {\mathrm {t}} ^ {\subset}. \tag {8}
$$
...
![](images/4157d3f5d7bccc1f8864db67eeaca86dbbfa38fd7b0fe4a6efc2e9f62f4c1c64.jpg)  
...
$$
\left( \begin{array}{l l} \boldsymbol {A} ^ {\subset} & \boldsymbol {B} ^ {\subset} \\ \boldsymbol {C} ^ {\subset} & \boldsymbol {D} ^ {\subset} \end{array} \right) \left( \begin{array}{l} \boldsymbol {x} ^ {\subset} \\ \boldsymbol {v} _ {\mathrm {t}} ^ {\subset} \end{array} \right) = \left( \begin{array}{l} \dot {\boldsymbol {x}} ^ {\subset} \\ \boldsymbol {i} _ {\mathrm {t}} ^ {\subset} \end{array} \right) \tag {9}
$$
...
$$
\left( \begin{array}{l l} \boldsymbol {P} ^ {\subset} & \boldsymbol {Q} ^ {\subset} \\ \boldsymbol {C} ^ {\subset} & \boldsymbol {D} ^ {\subset} \end{array} \right) \left( \begin{array}{l} \boldsymbol {x} ^ {\subset} \\ \boldsymbol {v} _ {\mathrm {t}} ^ {\subset} \end{array} \right) = \left( \begin{array}{l} \boldsymbol {\eta} _ {\boldsymbol {x}} ^ {\subset} \\ \boldsymbol {i} _ {\mathrm {t}} ^ {\subset} \end{array} \right) \tag {10}
$$
...
$$
L _ {P} ^ {\subset} U _ {P} ^ {\subset} \boldsymbol {x} ^ {\subset} = \eta_ {\boldsymbol {x}} ^ {\subset} - Q ^ {\subset} v _ {\mathrm {t}} ^ {\subset} \tag {11}
$$

where

$$
\boldsymbol {P} ^ {\subset} = L _ {\boldsymbol {P}} ^ {\subset} \boldsymbol {U} _ {\boldsymbol {P}} ^ {\subset}. \tag {12}
$$
...
$$
\boldsymbol {Y} _ {\mathrm {N}} ^ {\subset} = \boldsymbol {D} ^ {\subset} - \boldsymbol {T} _ {C P} ^ {\subset} \boldsymbol {Q} ^ {\subset} \tag {13}
$$

$$
\boldsymbol {j} _ {\mathrm {N}} ^ {\subset} = - \boldsymbol {T} _ {\boldsymbol {C P}} ^ {\subset} \boldsymbol {\eta} _ {\boldsymbol {x}} ^ {\subset} \tag {14}
$$

and the transfer matrix $\pmb { T } _ { C P } ^ { \subset }$

$$
\boldsymbol {T} _ {\boldsymbol {C P}} ^ {\mathbb {C}} \boldsymbol {P} ^ {\mathbb {C}} = \boldsymbol {C} ^ {\mathbb {C}}. \tag {15}
$$
...
![](images/1af0672e242c6b7c0e8b973e13753428b1d9d7773175030555ac24c7a33320a4.jpg)  
Fig. 8. Test circuit for evaluation of the child subdivision.

![](images/79a6d1976b72b1ad60178f945bc6fec8c83a64df650815e87e5dd3e784f2371e.jpg)  
...
This validation test is aimed at verifying the methodology when a child simulation process is formulated based on the state-space method while nodal analysis is used for the parent simulation. For the validation test, the terminal voltage $v _ { \mathrm { t } }$ is applied through an ideal voltage source as shown in Fig. 8. The considered lumped linear circuit in the subdivision is the fourth-order highpass ac filter of the CIGRE HVdc benchmark model and is shown in Fig. 9. The dynamic state variables are the currents through the inductors and the voltages across the capacitors $\pmb { x } ^ { \subset } = ( \bar { i } _ { \mathrm { L 1 } } i _ { \mathrm { L 2 } } v _ { \mathrm { C 1 } } v _ { \mathrm { C 2 } } ) ^ { \mathrm { T } }$ . The parameters are as follows:

$$
R _ {1} = 8 3. 0 0 \Omega , R _ {2} = 5 0. 0 0 \Omega , L _ {1} = 1 5. 1 0 \mathrm {m H},
$$

$$
L _ {2} = 9. 6 9 \mathrm {m H}, \quad C _ {1} = 6. 5 7 \mu \mathrm {F}, \quad C _ {2} = 7 8. 9 0 \mu \mathrm {F}.
$$

![](images/459afd0cff6f51d5a01e4756560514c87438f3a4132b98f6a24e57e2081c8d30.jpg)

![](images/597c64bc8d84d32a1945504b0097bc53d2ba732398abaad0215cc9beff4f554f.jpg)  
Fig. 10. Bode diagram for the fourth-order highpass filter; exact , simulated with method GENE .

The time step size of the simulation is equal to 50 s and, in the weight-averaged method discussed in Appendix B, $w = 1$ is selected to obtain the trapezoidal method because it is very well suited for simulating the sinusoidal waveforms.
...
![](images/88b511d9aaea6f765ed9b2b2e4d941ea5721f605ddf5cc4c94a1c63af3770fc8.jpg)  
valve voltage simulated with EMTP valve voltage simulated with GENE   
valve current simulated with EMTP

![](images/47c2fe80133a75a8380f15997320e4e67635387aac77316af456279ba45cfc95.jpg)  
valve current simulated with GENE

![](images/355eda96e079b38749a00e8b60765e9bf56ee8d1c4eea64dc220d6d88ffda6db.jpg)

![](images/6ce9b02f4a376101e5db51be50d2759c1fe15982739b22c86cfc918cd1cc1a4b.jpg)  
...
TABLE I MAXIMAL COMPUTATION TIMES PER TIME STEP   

<table><tr><td>GENE</td><td>minimum degree</td><td>full-matrix</td></tr><tr><td>30 μs</td><td>39 μs</td><td>300 μs</td></tr></table>
...
![](images/4757717dfd02170577f2f54f34657987c4651d0a6c89e0807b5bb2268a1079fc.jpg)  
...
$$
G _ {\mathrm {L}} = \frac {\tau (2 - w)}{2 L}, \quad \eta_ {\mathrm {L}} (k) = i _ {\mathrm {L}} (k - 1) + \frac {\tau w}{2 L} v _ {\mathrm {L}} (k - 1)
$$

the inductor is modeled through a companion model as shown on the right-hand side of Fig. 13. By setting $\mathbf { \dot { \iota } } _ { \mathrm { B } } = ( \dot { \iota } _ { \mathrm { B } 1 } \dot { \iota } _ { \mathrm { B } 2 } ) ^ { \mathrm { T } }$ , ${ \pmb v } _ { \mathrm { B } } ~ = ~ \overline { { ( } } \upsilon _ { \mathrm { B 1 } } ~ { \upsilon } _ { \mathrm { B 2 } } ) ^ { \mathrm { T } }$ and using the conventions indicated in Fig. $1 3 , Y _ { \mathrm { B } }$ and $j _ { \mathrm { B } }$ of (3) can be determined for the inductor example

$$
\boldsymbol {Y} _ {\mathrm {B}} = \left( \begin{array}{c c} G _ {\mathrm {L}} & - G _ {\mathrm {L}} \\ - G _ {\mathrm {L}} & G _ {\mathrm {L}} \end{array} \right), \quad \boldsymbol {j} _ {\mathrm {B}} = \left( \begin{array}{c} - \eta_ {\mathrm {L}} (k) \\ \eta_ {\mathrm {L}} (k) \end{array} \right). \qquad \text {(A . 3)}
$$
...
$$
\begin{array}{l} \frac {2 - w}{2} \left(\boldsymbol {A} ^ {\subset} \boldsymbol {x} ^ {\subset} (k) + \boldsymbol {B} ^ {\subset} \boldsymbol {v} _ {\mathrm {t}} ^ {\subset} (k)\right) + \frac {w}{2} \left(\boldsymbol {A} ^ {\subset} \boldsymbol {x} ^ {\subset} (k - 1) + \right. \\ \boldsymbol {B} ^ {\subset} \boldsymbol {v} _ {\mathrm {t}} ^ {\subset} (k - 1)) = \frac {\boldsymbol {x} ^ {\subset} (k) - \boldsymbol {x} ^ {\subset} (k - 1)}{\tau}. \tag {B.4} \\ \end{array}
$$

The GENE method requires (B.4) to be rearranged as follows:

$$
\boldsymbol {P} ^ {\subset} \boldsymbol {x} ^ {\subset} (k) + \boldsymbol {Q} ^ {\subset} \boldsymbol {v} _ {\mathrm {t}} ^ {\subset} (k) = \boldsymbol {\eta} _ {\boldsymbol {x}} (k).
$$

Comparison of this format with (B.4) gives

$$
\boldsymbol {P} ^ {\complement} = \left(\frac {2 - w}{2} \boldsymbol {A} ^ {\complement} - \bar {\boldsymbol {E}} _ {\kappa} / \tau\right) \tag {B.5}
$$

$$
\boldsymbol {Q} ^ {\complement} = \frac {2 - w}{2} \boldsymbol {B} ^ {\complement} \tag {B.6}
$$

$$
\begin{array}{l} \pmb {\eta} _ {\pmb {x}} = - \left(\frac {w}{2} \pmb {A} ^ {\complement} + \pmb {E} _ {\kappa} / \tau\right) \pmb {x} ^ {\complement} (k - 1) \\ - \frac {w}{2} \boldsymbol {B} ^ {\subset} \boldsymbol {v} _ {\mathrm {t}} ^ {\subset} (k - 1). \tag {B.7} \\ \end{array}
$$
...
$$
\boldsymbol {Y} _ {\mathrm {N}} ^ {\subset} = \boldsymbol {Y} _ {\mathrm {t t}} ^ {\subset} - \boldsymbol {T} _ {\mathrm {t d}} ^ {\subset} \boldsymbol {Y} _ {\mathrm {d t}} ^ {\subset} \tag {C.8}
$$

$$
\boldsymbol {j} _ {\mathrm {N}} ^ {\subset} = \boldsymbol {j} _ {\mathrm {t}} ^ {\subset} - \boldsymbol {T} _ {\mathrm {t d}} ^ {\subset} \boldsymbol {j} _ {\mathrm {d}} ^ {\subset} - \boldsymbol {Y} _ {\boldsymbol {j} _ {\mathrm {N}} \boldsymbol {v} _ {\mathrm {e}}} ^ {\subset} \boldsymbol {v} _ {\mathrm {e}} ^ {\subset} \tag {C.9}
$$

and the transfer matrices $\pmb { T } _ { \mathrm { t d } } ^ { \subset } \pmb { Y } _ { j _ { \mathrm { N } } \pmb { v } _ { \mathrm { e } } } ^ { \subset }$

$$
\boldsymbol {T} _ {\mathrm {t d}} ^ {\subset} \boldsymbol {Y} _ {\mathrm {d d}} ^ {\subset} = \boldsymbol {Y} _ {\mathrm {t d}} ^ {\subset}, \tag {C.10}
$$

$$
\boldsymbol {Y} _ {\boldsymbol {j} _ {\mathrm {N}} \boldsymbol {v} _ {\mathrm {e}}} ^ {\subset} = \boldsymbol {Y} _ {\mathrm {t e}} ^ {\subset} - \boldsymbol {T} _ {\mathrm {t d}} ^ {\subset} \boldsymbol {Y} _ {\mathrm {d e}} ^ {\subset}. \tag {C.11}
$$

The elements of $\pmb { T } _ { \mathrm { t d } } ^ { \subset }$ are obtained using forward and backsubstitutions based on the factorization of $\bar { \pmb { Y } } _ { \mathrm { d d } } ^ { C }$ into $\pmb { L } _ { \mathrm { d d } } ^ { \subset }$ and $\pmb { U } _ { \mathrm { d d } } ^ { \subset }$ .
...
$\nu$ counter which indicates the type of a piecewise linear branch model for which instances exist in the subdivision $\nu \in \{ 0 , \ldots , \psi - 1 \}$ ;
...
Step 3:According to the status of the subdivision activate prestored $T _ { \mathrm { t d } } ^ { \mathrm { c } } , \ : Y _ { j _ { \mathrm { N } } v _ { \mathrm { e } } } ^ { \mathrm { c } ^ { \mathrm { c } } } , \ : Y _ { \mathrm { d e } } ^ { \mathrm { c } } , \ : Y _ { \mathrm { d t } } ^ { \mathrm { c } }$ ${ \mathbfcal { L } } _ { \mathrm { d d } } ^ { \scriptscriptstyle \subset } , { \mathbfcal { U } } _ { \mathrm { d d } } ^ { \scriptscriptstyle \subset }$ in sprs rasr sse $\mathbf { Y } _ { \mathrm { N } } ^ { \epsilon }$
...
Ste : Caceulate he orto soure $\pmb { j } _ { \mathrm { N } } ^ { \mathrm { c } } = \pmb { j } _ { \mathrm { t } } ^ { \mathrm { c } } - \pmb { T } _ { \mathrm { t d } } ^ { \mathrm { c } } \pmb { j } _ { \mathrm { d } } ^ { \mathrm { c } } - \pmb { Y } _ { j _ { \mathrm { N } } v _ { \mathrm { e } } } ^ { \mathrm { c } } \pmb { v } _ { \mathrm { e } } ^ { \mathrm { c } }$
...
$$
m _ {\mathrm {c h g}} ^ {\mathrm {G} \subset} = N _ {\mathrm {d}} ^ {\subset^ {2}} + 2 N _ {\mathrm {d}} ^ {\subset} N _ {\mathrm {t}} ^ {\subset}.
$$

Without the status indicator and access scheme, additional operations need to be performed. The factorization of $\pmb { Y } _ { \mathrm { d d } } ^ { \subset }$ into upper and lower triangular matrices takes tiplications. The calculations (C.10) invocations, and (C.8) is accomplished throu $( N _ { \mathrm { d } } ^ { \subset 3 } - N _ { \mathrm { d } } ^ { \subset } ) / 3$ mul-tipli-tipli-$\mathrm { \ddot { N } _ { d } ^ { C 2 } } N _ { \mathrm { t } } ^ { \breve { C } }$ $N _ { \mathrm { d } } ^ { \breve { \subset } } N _ { \mathrm { t } } ^ { \breve { \subset } 2 }$ cations. The total multiplication count for diakoptics alone then gives

$$
\begin{array}{l} m _ {\mathrm {c h g}} ^ {\mathrm {D} \subset} = \left(N _ {\mathrm {d}} ^ {\subset^ {3}} - N _ {\mathrm {d}} ^ {\subset}\right) / 3 + N _ {\mathrm {d}} ^ {\subset^ {2}} \\ + N _ {\mathrm {d}} ^ {\subset^ {- 2}} N _ {\mathrm {t}} ^ {\subset} + N _ {\mathrm {d}} ^ {\subset} N _ {\mathrm {t}} ^ {\subset^ {2}} + 2 N _ {\mathrm {d}} ^ {\subset} N _ {\mathrm {t}} ^ {\subset}. \\ \end{array}
$$
...
![](images/2bfdf0d41572d23e896faff6e82c34a0d194c846f3832aafc589dd657a5db5e5.jpg)
...
![](images/0cd588ed6b87c274c78aae32a5cca0ec925e8dcc791f3cfdf0849100f495cae7.jpg)
