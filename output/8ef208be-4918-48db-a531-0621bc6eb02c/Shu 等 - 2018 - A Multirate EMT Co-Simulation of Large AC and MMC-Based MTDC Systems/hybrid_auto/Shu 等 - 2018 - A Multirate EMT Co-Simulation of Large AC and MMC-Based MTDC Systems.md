...
M ODULAR multilevel converters (MMC) have receivedwide acceptance in high-voltage direct-current (HVDC) wide acceptance in high-voltage direct-current (HVDC) transmission schemes due to its additional benefits include simple realization of redundancy, low losses, and most importantly, easy extension to multi-terminal (MTDC) configurations [1]. Many MMC-based MTDC transmissions have been carried out in China in recent years, such as Zhoushan and Nanao Project [2]. Therefore, studying the interaction between MMC-based MTDC systems and AC systems has

This work was supported in part by the Major Program of the National Natural Science Foundation of China (51490683), the National Natural Science Foundation of China (51577097), and the Research Project on Key Technologies and Demonstration Application of Flexible Coordinated Control of Electromagnetic Loop Network in Metropolitan Power Grid with High Load Density under Grant GZHKJ 00000101 (Corresponding author: Q. Jiang)

Dewu Shu , Xiaorong Xie, Qirong Jiang and Gaopeng Guo are with the State Key Lab. of Power System, Department of Electrical Engineering, Tsinghua University, Beijing, 100084, China (e-mail: shudw13@mails.tsinghua.edu.cn, xiexr@tsinghua.edu.cn, qrjiang@tsinghua.edu.cn, guogaopeng@mail. tsinghua.edu.cn).

Ke Wang is with Guangzhou Power Supply Co. Ltd., Guangzhou, China.

become an important subject. Electromagnetic transient (EMT) simulation is widely used for this purpose due to its capability of capturing the detailed dynamics of both converters and large AC systems [3]-[4].
...
![](images/22f6d023a3700495bab47836658d0fb337fd89ee24a1d9b6dddcde69f0bdcff2.jpg)  
...
$$
\left\{ \begin{array}{l} z _ {f} ^ {l} = R _ {f} ^ {l} + j \omega L _ {f} ^ {l}, \\ y _ {f} ^ {l} = \left(R _ {s} ^ {l} + j \omega L _ {s} ^ {l}\right) ^ {- 1}, \end{array} \right. \tag {1}
$$
...
$$
\frac {d}{d t} i _ {f} = \left[ L _ {f} \right] ^ {- 1} \left(v _ {f} - R _ {f} i _ {f} - u _ {f}\right), \tag {2}
$$
...
$$
\begin{array}{l} v _ {f} \left(t _ {k} + i h\right) = \alpha^ {- 1} \left\{i _ {f} \left(t _ {k} + i h\right) \right. \tag {3} \\ + \beta i _ {f} [ t _ {k} + (i - 1) h ] \} + u _ {f} (t _ {k} + i h), \\ \end{array}
$$

where

$$
\left\{ \begin{array}{l} \alpha = 1 - e ^ {- R _ {f} h / L _ {f}}, \\ \beta = e ^ {- R _ {f} h / L _ {f}}. \end{array} \right. \tag {4}
$$

And $t _ { k }$ is the $k ^ { t h }$ steps of AC subsystem; h is the time step of the MTDC subsystem; $t _ { k } + i h , i = 1 , 2 . . . i$ n refers to $i ^ { t h }$ sub-step between $\left[ t _ { k } , t _ { k + 1 } \right]$ . At each sub-step, parameters of interfaces in the MTDC subsystems should be updated.

(3) is rewritten as the impedance and equivalent voltage term, i.e., $Z _ { f } , \nu _ { h , f } ( t _ { k } + i h ) ;$ :

$$
v _ {f} \left(t _ {k} + i h\right) = Z _ {f} i _ {f} \left(t _ {k} + i h\right) + v _ {h, f} \left(t _ {k} + i h\right), \tag {5}
$$

where

$$
\left\{ \begin{array}{l} Z _ {f} = \alpha^ {- 1}, \\ v _ {h, f} \left(t _ {k} + i h\right) = \alpha^ {- 1} \beta i _ {f} \left[ t _ {k} + (i - 1) h \right] \\ + u _ {f} \left(t _ {k} + i h\right) \end{array} \right. \tag {6}
$$

Finally, according to Kirchhoff’s laws, the updating equation for each MTDC subsystem is rewritten as:

$$
\left[ \begin{array}{c c} \tilde {G} _ {f} & I \\ - I & Z _ {f} \end{array} \right] \left[ \begin{array}{l} \tilde {v} _ {f} (t _ {k} + i h) \\ i _ {f} (t _ {k} + i h) \end{array} \right] = \left[ \begin{array}{l} \tilde {i} _ {h, f} (t _ {k} + i h) \\ v _ {h, f} (t _ {k} + i h) \end{array} \right], \tag {7}
$$
...
![](images/36a20c1feb3b65147057dc00955cbeb911921a13e23fa9656e7c1db3abf6b502.jpg)  
Fig. 2. Flow chart of the multirate co-simulation method.

is the node voltage vector inside the MTDC subsystem; $i _ { f } ( t _ { k } + i h )$ is the interface current.

Similarly, the updating equation for the AC subsystem is written as:

$$
\left[ \tilde {G} _ {s} + G _ {s} \right] v _ {s} \left(t _ {k}\right) = \tilde {i} _ {h, s} \left(t _ {k}\right) + i _ {h, s} \left(t _ {k}\right), \tag {8}
$$
...
For the AC subsystem, all its neighboring DC subsystems are replaced with time-varying Norton equivalents, the parameters of which are calculated by averaging the results of the previous iteration. During the current step, only a single

iterative operation is required to advance the simulation to the next time interval, or from $t _ { k } \mathrm { ~ t o ~ } t _ { k + 1 }$ , where $t _ { k } , \ t _ { k + 1 }$ refer to the simulation time of $k ^ { t h }$ and $k + 1 ^ { t h }$ steps in the AC subsystem.

Meanwhile, for the DC subsystems, their interface models are represented with time-varying Thevenin equivalents. The simulation from $t _ { k }$ to $t _ { k + 1 }$ are divided into n sub-steps, namely tk + ih, $i = 1 , 2 , . . . n .$ At each subs-step, the parameters of the interface model are predicted by moving-window prediction.
...
As shown in Fig. 3, a moving window consisting of m latest Thevenin voltages, or $u _ { f } ( t _ { k } - m ) , \dots u _ { f } ( t _ { k } - 1 )$ , is employed

![](images/9a32728203b586bc5e779d2f2e174ce2684f2b2e507d391e68dd4c35a13e27d6.jpg)  
Fig. 3. Illustration of Moving-window prediction and linear prediction.

first to get the following piecewise cubic spline functions during the n sub-steps:

$$
\begin{array}{l} F _ {i} (t) = \frac {\left[ t _ {k} + (i - 1) h - t \right] ^ {3} M _ {i - 2} + \left[ t - t _ {k - 1} - (i - 2) h \right] ^ {3} M _ {i - 1}}{6 h} + \\ \frac {[ t _ {k} + (i - 1) h - t ] u _ {f} [ t _ {k} + (i - 2) h ] + [ t ^ {- n} t _ {k - 1} - (i - 2) h ] u _ {f} [ t _ {k} + (i - 1) h ]}{h} \tag {9} \\ - \frac {h \left\{\left[ t _ {k} + (i - 1) h - t \right] M _ {i - 2} + \left[ t - t _ {k} - (i - 2) h \right] M _ {i - 1} \right\}}{6}, i = 1, 2 \dots n, \\ \end{array}
$$
...
$$
\tilde {u} _ {f} \left(t _ {k} + i h\right) = F _ {i} \left(t _ {k} + i h\right), i = 1, 2 \dots n, \tag {10}
$$

The errors of Thevenin voltages are quantified as

$$
\tilde {e} \left(t _ {k} + i h\right) = \left| \left| u _ {f} \left(t _ {k} + i h\right) - \tilde {u} _ {f} \left(t _ {k} + i h\right) \right| \right|. \tag {11}
$$

According to [24],

$$
\tilde {e} \left(t _ {k} + i h\right) = \leq \frac {h ^ {4}}{4 !} \max  | | u _ {f} ^ {(4)} \left(t _ {k} + i h\right) | |, i = 1, 2 \dots n. \tag {12}
$$
...
$$
\begin{array}{l} u _ {f} ^ {l} \left(t _ {k + 1}\right) = \tilde {u} _ {f} ^ {l} \left(t _ {k + 1}\right) + \Delta u _ {f} ^ {l} \left(t _ {k + 1}\right) \tag {13} \\ = \bar {u} _ {l} ^ {f} \left(t _ {k + 1}\right) + \sum_ {m = 1, m \neq l} ^ {N} Z _ {l m} i _ {f} ^ {m} \left(t _ {k + 1}\right), \\ \end{array}
$$
...
$$
i _ {s} \left(n _ {k + 1}\right) = \frac {1}{n} \sum_ {m = 0} ^ {n - 1} i _ {f} \left(t _ {k} + m h\right), \tag {14}
$$
...
$$
Z _ {a c} = \frac {\hat {u}}{\hat {i}} = \frac {\left(R + \frac {2 L}{n h}\right) + \left(R - \frac {2 L}{n h}\right) z ^ {- n}}{1 + z ^ {- n}}, \tag {17}
$$

where $\hat { u } , \hat { i }$ are the Fourier transform of the voltage and current and $z = e ^ { j \omega h }$ .

However, if the same RL branch is placed in the MTDC subsystem, after discretized within the same time interval $\left[ t _ { k } , \ t _ { k + 1 } \right]$ , it changes to

$$
\begin{array}{l} \frac {1}{2} \left[ u \left(t _ {k + 1}\right) - R i \left(t _ {k + 1}\right) + u \left(t _ {k}\right) - R i \left(t _ {k}\right) \right] \\ + \sum_ {m = 1} ^ {n - 1} [ u (t _ {k} + m h) - R i (t _ {k} + m h) ] = \frac {L}{h} [ i (t _ {k + 1}) - i (t _ {k}) ]. \tag {18} \\ \end{array}
$$

A similar impedance in the MTDC subsystem is derived as

$$
Z _ {d c} = \frac {\hat {u}}{\hat {i}} = \frac {\frac {L}{h} \left(z ^ {n} - 1\right) + \frac {R}{2} \left(1 + z ^ {n}\right) + R \sum_ {m = 1} ^ {n - 1} z ^ {m}}{\frac {1}{2} \left(1 + z ^ {n}\right) + \sum_ {m = 1} ^ {n - 1} z ^ {m}}. \tag {19}
$$

Then, the impedance ratio is defined as $\varsigma ( n , \omega ) = Z _ { a c } / Z _ { d c }$ and when $R \to 0 ,$ , it can be simplified into

$$
\lim  _ {R \rightarrow 0} \varsigma (n, \omega) = [ (1 + z ^ {n}) + 2 \sum_ {m = 1} ^ {n - 1} z ^ {m} ] / [ n \cdot (1 + z ^ {n}) ]. \tag {20}
$$
...
$$
0. 9 \leq | \varsigma (n, \omega) | \leq 1. 1, \quad \omega \in [ 0, 0. 2 f _ {N y} ], \tag {21}
$$

where $f _ { N y }$ is the Nyquist frequency; and the maximum frequency concerned is $0 . 2 f _ { N y }$ [31]. It should be noted that

![](images/e5e463ddfe00952b268408c1ccc76fd20f90c5ad3fe1c3c44811e721c8cd8424.jpg)  
...
![](images/66044fa90a4f8b01c5ab1640628d398004831fd258a635abc047c9ca2752b9c4.jpg)

![](images/49858a7cb8f07f41bfdb903dadb1d046e77b378298962e7e09802c4f1128667a.jpg)  
${ \mathrm { F i g . } }$ 6. The impedance ratio varying with rate ratio and frequency.
...
![](images/e1f79c1452a6020ceee61088fecc42badfb13362368fc949d483a29edc7259f5.jpg)

![](images/c20235a973974a66bf7d278ff978772ab218beaed2026a4c31db02f6fd16ce53.jpg)  
Fig. 7. Voltages obtained with different updating techniques for the Thevenin Equivalents.

![](images/320d8b35094fc96e3f1466b3007bd956d9205831f572b6dd4e24ede36cee3b00.jpg)

![](images/1ce689aa22e0670c608f3b235d58e09a941d8cc1336a622aeac425587d54f57f.jpg)  
...
![](images/c00c66c767c19c55866b378f887c91b88a8487941abf06f7feae515c0c4c783c.jpg)

![](images/c5be8ae579ef340e46dc3efa3176494191ee17529a23348b12d943dd140add2b.jpg)

![](images/7aad32f59a00e00df6d0349e6e10b1662473199e39e107e42518f9e1529c8120.jpg)  
...
![](images/c102954a01a9230b0b829a1971d726c6d4c6b02083c0503688fdf2180d6652dc.jpg)

![](images/9e0e64565a46e8249dab80db1e638d96008ee2784e0114eed4ad46121e696bd2.jpg)

![](images/b3f86efcf08a4b57c6121fc8249d57be1fe2dbc19ba6fc010a81a0adf594d854.jpg)

![](images/b7ec503fd9d6db87b975df286f32711a4b6527be42d8e70e3236d952396eebd8.jpg)

![](images/62b559628498686f5286c8c0db61c5257bf3d313c0772eeb81bd6107518f1d40.jpg)  
Fig. 10. DC current (short interface lines).

![](images/223334394fc41177465a747be07b09430151ac9a3196874ed31450b7d1e9a1d5.jpg)

![](images/2f10e24b4346c2783a3734dce3084f4508a8957aff26a33ab27213e8236194b4.jpg)

![](images/b7e6dd1cca157d18ed053b9a474b47aaae07103300af49bb815c864f3cc4e0ea.jpg)

![](images/e65e966702e84146fa4ab3c5aeabbc5ecc0959de20d2c0c0be0922066eb3015e.jpg)  
Fig. 11. AC voltage and DC voltage of MMC1(short interface lines)

![](images/2cf34025b8f9e4805f326e742a1ac0b311104d32c1ace75c40a89d276b382b9b.jpg)

![](images/7cb3ca7a11ac997650a21c5c5e18318ec389f5b43952107861da3aa3bce54c84.jpg)

![](images/db911e3788a1bfeb8204ed90663f7bb8ffb9f8eb48259fdfb851a2c9721ad1ac.jpg)

![](images/2c3281f092573a2be82dfb017870ace4e06137ea81e9f63273030808dacfa0f5.jpg)  
Fig. 12. AC voltage and DC voltage of MMC1(long interface lines)

![](images/abd679b53b86243b9a6510febd1ccc90668217fe59138fe7528cb838ed02be17.jpg)

![](images/d6a2a1233fdb77513fdc4b202b7f772527dac8419673f26c1f93e12e5b06f4cc.jpg)

![](images/23440aaf7c1592e72ca0f70c2874b7fedd87c7565765be2ec763e244bb3905a2.jpg)

![](images/72c11498da075a9e4739a4546146effb8397baf3ef057905d84471b1d7f36370.jpg)  
Fig. 13. Arm voltages and capacitor voltages of MMC1(short interface lines)

![](images/400b7d321bfa2bb18b01fcccb0738951d9e49b84a7d65aa62ea4d315c808184c.jpg)

![](images/bbeabd43cc3bcc52888bcfc8366ebe85bd9cd0a66c4789ed0bb637ff94c418a6.jpg)

![](images/f3b422d270c42fdf56d84fe63bc614b0d24986184c4ec59aaf4c457ccb2cc501.jpg)  
Fig. 14. Arm voltages and capacitor voltages of MMC1(long interface lines)

TABLE I COMPARISON OF CPU TIME AND AVERAGE SIMULATION ERRORS (UNIT: IN PU) WITH DIFFERENT RATE RATIOS (PROPOSED METHOD:SHORT INTERFACE LINE)   

<table><tr><td>Rate ratio n</td><td>CPU Time/s</td><td>AC subsystem 
variables</td><td colspan="2">Interface variable</td><td colspan="2">DC subsystem variables</td><td colspan="3">DC subsystem MMC variables</td></tr><tr><td></td><td></td><td>U8A</td><td>U10A</td><td>I9-10A</td><td>Idc-MMC1</td><td>Udc-MMC1</td><td>Uap-MMC1</td><td>Iarm-MMC1</td><td>Uc-MMC1</td></tr><tr><td>1</td><td>5925</td><td>/</td><td>/</td><td>/</td><td>/</td><td>/</td><td>/</td><td>/</td><td>/</td></tr><tr><td>2</td><td>2370</td><td>0.006</td><td>0.013</td><td>0.023</td><td>0.011</td><td>0.011</td><td>0.005</td><td>0.005</td><td>0.005</td></tr><tr><td>3</td><td>799</td><td>0.012</td><td>0.018</td><td>0.026</td><td>0.017</td><td>0.012</td><td>0.011</td><td>0.006</td><td>0.008</td></tr><tr><td>4</td><td>680</td><td>0.017</td><td>0.024</td><td>0.032</td><td>0.019</td><td>0.013</td><td>0.017</td><td>0.009</td><td>0.009</td></tr><tr><td>5</td><td>530</td><td>0.023</td><td>0.031</td><td>0.039</td><td>0.022</td><td>0.014</td><td>0.021</td><td>0.012</td><td>0.012</td></tr><tr><td>6</td><td>450</td><td>0.050</td><td>0.037</td><td>0.043</td><td>0.026</td><td>0.017</td><td>0.026</td><td>0.014</td><td>0.013</td></tr></table>

TABLE II COMPARISON OF CPU TIME AND AVERAGE SIMULATION ERRORS (UNIT: IN PU) WITH DIFFERENT RATE RATIOS (TLM METHOD:SHORT INTERFACE LINE)   

<table><tr><td>Rate ratio n</td><td>CPU Time/s</td><td>AC subsystem 
variables</td><td colspan="2">Interface variable</td><td colspan="2">DC subsystem variables</td><td colspan="3">DC subsystem MMC variables</td></tr><tr><td></td><td></td><td>U8A</td><td>U10A</td><td>I9-10A</td><td>Idc-MMC1</td><td>Udc-MMC1</td><td>Uap-MMC1</td><td>Iarm-MMC1</td><td>Uc-MMC1</td></tr><tr><td>1</td><td>5925</td><td>/</td><td>/</td><td>/</td><td>/</td><td>/</td><td>/</td><td>/</td><td>/</td></tr><tr><td>2</td><td>2573</td><td>0.014</td><td>0.023</td><td>0.037</td><td>0.012</td><td>0.014</td><td>0.009</td><td>0.006</td><td>0.006</td></tr><tr><td>3</td><td>811</td><td>0.021</td><td>0.044</td><td>0.042</td><td>0.018</td><td>0.015</td><td>0.019</td><td>0.007</td><td>0.008</td></tr><tr><td>4</td><td>694</td><td>0.027</td><td>0.058</td><td>0.052</td><td>0.020</td><td>0.016</td><td>0.023</td><td>0.010</td><td>0.009</td></tr><tr><td>5</td><td>520</td><td>0.040</td><td>0.074</td><td>0.063</td><td>0.023</td><td>0.018</td><td>0.028</td><td>0.013</td><td>0.012</td></tr><tr><td>6</td><td>474</td><td>0.084</td><td>0.092</td><td>0.070</td><td>0.028</td><td>0.021</td><td>0.031</td><td>0.018</td><td>0.012</td></tr></table>
...
![](images/b069d1feaa9360703ba312593ad7a115353fa453fded049f0e7aa869d444049d.jpg)  
Fig. 15. Power curves of G8.

![](images/dc3906a0697f865540e80742ee4d712d3af4c5d095fafde531c842d654391282.jpg)  
Fig. 16. Angular velocity of G8.

To quantify the accuracy, a metric called average simulation error is defined as

$$
E _ {\text {a v g}} = \frac {1}{N} \sum_ {i = 1} ^ {N} | x _ {i, \mathrm {m}} - x _ {i, \mathrm {u}} |, \tag {22}
$$
...
TABLE III SELF- AND MUTUAL IMPEDANCES OF INTERFACES   
...
Fig. 20 shows the $E _ { a \nu g }$ versus magintudes of self-/mutual impedance curves with/without the stepwise correction technique. As can be seen, with the stepwise corection technique, errors of interface voltages have been reduced significantly. Moreover, $E _ { a \nu g }$ will increase as the magnitude of self-/mutualimpedances drops. That is to say, when the magnitude of self-/mutual- impedances is small, the interactions between

![](images/f6afd6b8d210dfc383d4be7877195bc0bb547f2c190df5372a280faf7b563bb1.jpg)  
Fig. 19. Voltages obtained with/without stepwise correction technique.

![](images/38c8ee55bad5ab02de538fb9d9f3f79ebcb7718a32a9af6105310804b7310927.jpg)  
Fig. 20. $E _ { a \nu g }$ versus self-/mutual- impedance curves with/without stepwise correction technique.

interfaces cannot be neglected. Thus, the stepwise correction technique is important to improve the accuracy of the interfaces.
...
In this paper, a multirate EMT co-simulation method is proposed to improve the simulation efficiency of large AC and MMC-based multi-terminal DC (MTDC) systems. It partitions

TABLE IV COMPARISONS BETWEEN THE PROPOSED METHOD AND THE SSN METHOD   

<table><tr><td>Method</td><td>CPU Time/s</td><td>U10A</td><td>I9-10A</td><td>Udc-MMC1</td><td>Uc-MMC1</td></tr><tr><td>SSN</td><td>960</td><td>0.112</td><td>0.903</td><td>0.045</td><td>0.085</td></tr><tr><td>proposed</td><td>450</td><td>0.037</td><td>0.043</td><td>0.017</td><td>0.013</td></tr></table>

the whole system into multiple AC and MTDC subsystems and assigns them different time steps. To coordinate the simulation of different subsystems, we develop a cross-platform framework, in which their interfaces are modelled as timevarying Thevenin and Norton equivalents. The parameters of the equivalents are updated through moving-window prediction, stepwise correction and averaging techniques. Further, by discretizing the interface model with RM algorithm and selecting the best rate ratio, the numerical accuracy and stability of the co-simulation method is guaranteed Variables inside each subsystem and interface variables are obtained simultaneously by the proposed augmented network equations (ANEs).
...
TABLE V PARAMETERS OF THE MTDC SUBSYSTEM   
