...
(1．可再生能源与工业节能安徽省工程实验室(合肥工业大学)，安徽省 合肥市 230009；2．中国电力科学研究院有限公司，北京市 海淀区 100192)
...
$$S_i = \begin{cases} 1, & v_m > v_c \\ 0, & v_m \le v_c \end{cases} \quad (1)$$

式中 $i=a, b, c$ 分别代表 A、B、C 三相。

考虑低频分量时，变流器非连续系统状态的微分方程为：

$$\dot{x}(t) = f(x) + b u(d(x) - \text{tri}(t, T)) \quad (2)$$

式中 $u$ 为系统输入函数，与 $d(x)$ 及 $\text{tri}(t, T)$ 有关。

对于特定开关电路的非连续系统，给定 $t_0$ 时刻状态变量值 $x(t_0)$，将式(2)改写为

$$x(t) = x(t; t_0, x(t_0)) \equiv x(t_0) + \int_{t_0}^{t} \left[ f(x(s)) + \sum_{i=1}^{N} f_i(x(s)) \cdot b u(d_i(x(s)) - \text{tri}(s, T)) \right] ds \quad (3)$$

式中 $N$ 为开关组数。

由开关函数 $S_i$ 和开关周期 $T_s$ 可以得到周期性开关函数占空比 $d_i$。

$$d_i = \frac{1}{T_s} \int_{t-T_s}^{t} S_i(\tau) d\tau, \quad i = a, b, c \quad (4)$$

周期性开关函数的占空比在每一开关周期 $T_s$ 上是恒定的，因此将状态变量用平均值 $y$ 表示为

$$y(t) = y(t; t_0, y(t_0)) \equiv y(t_0) + \int_{t_0}^{t} f_0(y(s)) ds + \sum_{i=1}^{N} \int_{t_0}^{t} f_i(y(s)) d_i(y(s)) ds \quad (5)$$

式(4)、(5)即为变流器状态空间平均模型，与占空比 $d_i$ 有关，是对开关导通和关断时 2 组不同的线性状态方程进行状态平均的非线性模型。

### 1.2 分段状态空间平均法

文献[19,22]介绍了分段状态空间平均法(piecewise state space averaging，P-SSA)，其核心是将仿真时间分解成合理的子区间。与传统平均化建模