...
This paper is based on the ULM formulation [3]. For a multiphase transmission system, contains different propagation modes with different velocities. As explained in [3], is first decomposed into modes, then a proper time delay is assigned to each mode. Finally, these modes