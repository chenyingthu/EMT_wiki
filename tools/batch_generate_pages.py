...
def generate_page_content(page_name, page_type, sources_content):
    ...
    # 生成定义与边界
    definition = all_abstracts[0] if all_abstracts else f'{page_name}是 EMT Wiki 中待进一步核实的概念入口。'

    # 生成EMT作用
    emt_role = f'当前仅能确认 {page_name} 与 EMT 仿真相关，具体作用需要结合相关来源进一步核实。'
    if all_methods:
...
**边界限定**：本页由批量脚本生成，仅保留最小入口信息；术语定义、适用对象和失败边界均需人工复核。

## EMT中的作用

{emt_role[:400]}

当前页面不默认推断“主要应用”。如需写明控制、保护、故障或加速用途，必须由相关 source 页直接支撑。

## 主要分支与机制

- 待补充：需要基于来源页确认是否属于方法、模型、主题入口，避免误贴无关机制。

## 形式化表达

- 待补充：无可靠公式来源时，不自动插入公式。

{equations_section}

## 适用边界与失败模式

- 待补充：需要结合来源页给出假设、限制和不适用场景。
...
*本页面由批量生成脚本创建，仅用于知识图谱占位；必须进一步人工审查和完善。*
"""
