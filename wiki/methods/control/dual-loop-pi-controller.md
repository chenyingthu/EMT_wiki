...
该方法页讨论级联 PI 结构本身，不把它写成所有变流器的“标准最优控制”。双闭环 PI 主要属于 VSC/逆变器控制范畴；LCC-HVDC 的晶闸管触发角和关断角控制应放在 [[thyristor-control]] 和 [[extinction-angle-calculation]] 中。
...
[[dynamic-performance-of-embedded-hvdc-with-13&14]] 明确使用常规 dq 双环结构：外环根据直流电压、有功功率或交流电压目标生成 d/q 电流参考，内环跟踪电流并生成调制信号。论文还在有功参考层叠加频率相关补偿，用于嵌入式 HVDC 的功率振荡分担。该来源可支撑“双环 PI 作为 VSC-HVDC 控制骨架”的场景化表述，但其暂态稳定收益只适用于该 PSCAD/EMTDC 算例和同步交流网络内的嵌入式 HVDC 条件。

[[characteristic-analysis-of-high-frequency-resonance-of-flexible-high-voltage-dir]] 将 MMC 内部动态、PLL、内外环、环流抑制和延时并入阻抗模型，用于解释高频振荡。它提醒双闭环 PI 不能脱离 PLL、延时和主电路动态单独评价稳定性。
...
| 来源 | 证据用途 | 边界 |
|------|----------|------|
| [[dynamic-performance-of-embedded-hvdc-with-13&14]] | VSC-HVDC 常规 dq 双环 PI 结构及有功参考层附加控制 | 稳定性结论限于嵌入式 HVDC PSCAD 算例 |
| [[characteristic-analysis-of-high-frequency-resonance-of-flexible-high-voltage-dir]] | MMC 高频阻抗模型中 PLL、内外环、环流控制和延时共同影响稳定性 | 面向 MMC-HVDC 高频振荡，不代表所有 VSC |
| [[pi-controller-model]] | PI 的连续、离散和抗饱和模型背景 | 该模型页含较多参数示例，引用时需核验来源 |

## 与相关页面的关系

- [[pi-controller-model]] 给出单个 PI 控制器模型。
- [[vector-control-model]] 和 [[dq-transformation]] 给出坐标变换背景。
- [[pll-model]] 决定跟网型控制的同步参考。
- [[vsc-model]]、[[mmc-model]] 和 [[inverter-model]] 决定被控对象边界。
