...
HBSM 在 EMT 仿真中的关键作用：
- ** fault analysis**：HBSM 无法阻断直流故障电流，故障时所有 SM 电容通过二极管向故障点放电，形成反向电流
- **switching transient**：开关级模型需精确捕捉 IGBT/二极管切换瞬间的电压/电流过冲
- **capacitor voltage ripple**：需跟踪每个 SM 电容电压的动态变化，用于排序和均压控制
- **system-level scalability**：戴维南等效等加速模型使计算量随 SM 数线性增长，而非指数增长
...
**5. 半桥三态定义（Zhang 2023）**
- 状态 0（$T_{OFF}/D_{OFF}$）：IGBT 关闭、二极管关闭
- 状态 1（$T_{OFF}/D_{ON}$）：IGBT 关闭、二极管导通
- 状态 2（$T_{ON}/D_{OFF}$）：IGBT 导通、二极管关闭
