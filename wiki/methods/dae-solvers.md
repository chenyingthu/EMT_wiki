...
```mermaid
graph TD
    subgraph Ncmp[DAE 求解器 (DAE Solvers)]
        N0[固定步长隐式 DAE: 每步用固定 $\Delta t$…]
        N1[变步长/变阶 DAE: 根据误差估计调整步长或阶数]
        N2[分区 DAE: 把网络、控制器、设备模型或子系统分开迭代]
        N3[固定导纳伴随模型: 把设备整理成固定导纳和历史源]
        N4[多速率 DAE: 不同子系统使用不同步长并交换接口量]
    end
```
