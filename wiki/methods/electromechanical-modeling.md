...
```mermaid
graph TD
    subgraph Ncmp[机电暂态建模 (Electromechanical Mo…]
        N0[经典模型: $E'$ 近似恒定，忽略励磁和阻尼绕组细节]
        N1[两轴/暂态模型: 保留 d/q 轴暂态电势和电抗差异]
        N2[次暂态/Park 模型: 保留更快电磁状态和阻尼绕组近似]
        N3[动态负荷模型: 感应电机、恢复负荷或复合负荷]
        N4[机电-EMT 接口模型: 外部系统相量/序分量等值，内部…]
    end
```
