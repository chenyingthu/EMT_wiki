...
```mermaid
graph TD
    subgraph Ncmp[模态分析 (Modal Analysis)]
        N0[LTI 模态分析: 对运行点线性化 A 矩阵求左右特征向量]
        N1[离散模态分析: 对采样矩阵或状态转移矩阵求模态]
        N2[Floquet 模态分析: 对周期轨迹的单周期状态转移矩…]
        N3[频域模态解释: 对导纳/阻抗矩阵或闭环增益矩阵分析谱结构]
        N4[测量驱动模态辨识: 从 PMU/EMT 响应估计频率、阻…]
    end
```
