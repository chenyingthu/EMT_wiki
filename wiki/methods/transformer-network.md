...
```mermaid
graph TD
    subgraph Ncmp[变压器网络建模 (Transformer Network…]
        N0[理想变压器: 匝比、相移、功率守恒]
        N1[漏抗加励磁支路: 绕组电阻、漏感、励磁电感和铁损]
        N2[三相耦合模型: 连接组别、相间互感、零序路径]
        N3[多绕组矩阵模型: 多端口阻抗或导纳矩阵]
        N4[宽频端口模型: 频率相关 $\mathbf{Y}(s)$…]
        N5[分布式磁路模型: 磁阻网格、绕组分段、局部饱和]
    end
```
