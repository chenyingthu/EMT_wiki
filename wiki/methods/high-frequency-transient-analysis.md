...
```mermaid
graph TD
    subgraph Ncmp[高频暂态分析 (High-frequency Trans…]
        N0[架空线/配电线: 地模损耗、分支反射、外部场耦合]
        N1[地下/海底电缆: 护套/铠装、介质损耗、短线步长约束]
        N2[接地系统: 冲击阻抗、频变土壤、GPR、多端口耦合]
        N3[避雷器/SPD: 非线性残压、能量、引线电感]
        N4[变压器/GIS: 端口杂散参数和内部波过程]
    end
```
