...
本页面关注CPU共享内存多线程在EMT中的应用，涵盖线程级任务划分、稀疏矩阵并行求解、网络分区接口并行化和多换流器并行更新等典型场景。
...
在多插槽服务器上，线程跨NUMA节点访问远端内存延迟增加3-5倍。OpenMP提供两种线程绑定策略：

$$
\text{CPU\_hot\_set} = \text{OMP\_PROC\_BIND} \in \{\text{spread}, \text{close}, \text{master}\}
$$

- **spread**：将线程均匀散布在各NUMA节点，适合负载均衡优先场景
- **close**：将线程绑定在最近节点，适合内存带宽敏感场景
- **first-touch**：页分配在首次访问时落在本地节点，需配合 `posix_fadvise` 使用

$$
t_{\text{remote}} \approx 3\text{-}5 \times t_{\text{local}}, \quad \text{NUMA带宽比} \approx 0.3\text{-}0.5 \times \text{本地带宽}
$$
...
多线程并行填充稀疏矩阵时，为避免写冲突，采用**局部累加-合并**策略：

$$
\mathbf{Y}_{\text{local}}^{(t)} = \sum_{e \in \text{elements assigned to thread } t} \mathbf{A}_e, \quad \mathbf{Y}_k = \sum_{t=1}^{N_{\text{threads}}} \mathbf{Y}_{\text{local}}^{(t)}
$$

其中 $\mathbf{A}_e$ 为元件 $e$ 对节点导纳矩阵的贡献，$N_{\text{threads}}$ 为线程数。
...
$$
\mathbf{B}_i \mathbf{v}_i = \mathbf{i}_i - \mathbf{G}_i \mathbf{v}_b, \quad i = 1, 2, \cdots, n
$$

并行前代回代按拓扑顺序执行，关键路径长度为 $O(\log n)$（$n$ 为块数）：

$$
\mathbf{v}_b^{(\text{new})} = \left(\mathbf{C} - \sum_{i=1}^{n} \mathbf{H}_i^T \mathbf{B}_i^{-1} \mathbf{G}_i \right)^{-1} \left( \mathbf{i}_b - \sum_{i=1}^{n} \mathbf{H}_i^T \mathbf{B}_i^{-1} \mathbf{i}_i \right)
$$
...
$$
\begin{bmatrix} \mathbf{i}_{\text{eq},1} \\ \mathbf{i}_{\text{eq},2} \\ \vdots \\ \mathbf{i}_{\text{eq},m} \end{bmatrix} = \begin{bmatrix} \mathbf{G}_{\text{eq},11} & \mathbf{G}_{\text{eq},12} & \cdots & \mathbf{G}_{\text{eq},1m} \\ \mathbf{G}_{\text{eq},21} & \mathbf{G}_{\text{eq},22} & \cdots & \mathbf{G}_{\text{eq},2m} \\ \vdots & \vdots & \ddots & \vdots \\ \mathbf{G}_{\text{eq},m1} & \mathbf{G}_{\text{eq},m2} & \cdots & \mathbf{G}_{\text{eq},mm} \end{bmatrix} \begin{bmatrix} \mathbf{v}_{\text{port},1} \\ \mathbf{v}_{\text{port},2} \\ \vdots \\ \mathbf{v}_{\text{port},m} \end{bmatrix} + \begin{bmatrix} \mathbf{i}_{\text{hist},1} \\ \mathbf{i}_{\text{hist},2} \\ \vdots \\ \mathbf{i}_{\text{hist},m} \end{bmatrix}
$$

其中 $m$ 为VSC数量，$\mathbf{G}_{\text{eq},ij}$ 反映换流器 $j$ 端口电压对换流器 $i$ 端口电流的耦合导纳。
