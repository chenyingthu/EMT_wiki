...
```mermaid
graph TD
    subgraph Ncmp[分层接入 (Layered Connection)]
        N0[电压等级分层接入: 直流或换流站接入不同交流电压层]
        N1[双层频变网络等值: 详细层加外部等值层]
        N2[分区混合仿真接口: EMT 区域与机电区域]
        N3[多馈入等值边界: 多个换流器或多端口交流网络]
        N4[分层控制接入: 多换流器或多层电压控制]
    end
```
