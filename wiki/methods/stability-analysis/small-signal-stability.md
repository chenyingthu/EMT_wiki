...
```mermaid
graph TD
    subgraph Ncmp[小信号稳定性 (Small-Signal Stabili…]
        N0[同步机与励磁/PSS: 机电振荡是否有足够阻尼]
        N1[电力电子并网装置: PLL、电流环、电压环和网络阻抗是否…]
        N2[周期开关系统: 周期稳态附近扰动是否跨周期衰减]
        N3[线路和频变网络: 频变传播、端口导纳和控制器是否耦合出弱…]
    end
```
