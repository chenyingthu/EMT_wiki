...
title: "电力系统稳定器 (PSS)"
type: method
tags: [pss, power-system-stabilizer, damping, excitation, oscillation, small-signal-stability, ieee-pss, anderson-pss, anderson]
created: "2026-05-02"
updated: "2026-05-15"
---
...
电力系统稳定器（Power System Stabilizer, PSS）是同步发电机 [[excitation-system]] 励磁系统中的附加阻尼控制环节。PSS 从转速偏差 $\Delta\omega$、频率偏差 $\Delta f$、有功功率偏差 $\Delta P$ 或加速功率 $\Delta P_a$ 等信号中提取机电振荡分量，生成与转速偏差同相位的附加电压调节输入 $V_{PSS}$，注入 AVR 输入端，通过改变电磁转矩的相位抑制目标振荡模态上的低频功率振荡。

从物理机制看，PSS 并不改变同步机的稳态电压设定值——那是 AVR 的职能。PSS 的作用是使电磁转矩的相位超前转子角度偏差，在相位滞后最严重的振荡频率点（通常为 0.2~2.0 Hz）产生与转速偏差同相位的转矩分量，从而实现阻尼。IEEE Std 421.5-2005 定义了三种标准 PSS 结构（IEEE PSS1A/2A/3A），Anderson PSS 和 Widerholm PSS 等变体在工业界广泛应用。

**PSS 是控制方法，不是 EMT 核心求解方法。** 它主要服务于机电振荡和小信号稳定分析；在 EMT 中，PSS 必须作为控制器模型离散化并接入同步机励磁系统。PSS 不能替代开关暂态、谐波、保护动作或宽频网络响应的 EMT 建模。更通用的同名规范页见 [[power-system-stabilizer]]。
...
其中 $T_W$ 为隔直时间常数，通常取 1.0~10.0 s。隔直环节在稳态（$s\to 0$）时增益趋于 0，在振荡频段（$s=j\omega$）时增益接近 1。
...
典型值为 $T_1/T_2 = 0.1/0.02$ 或 $0.3/0.06$（s），$n=1$ 或 $n=2$。参数整定需绑定具体机组、系统模态和验证方法——同一参数在不同机组上可能产生正阻尼或负阻尼。
...
其中 $K_{p1}$ 为功率偏差通道的增益系数。这种结构的优点是能在电功率测量噪声较大时保持较好的阻尼效果。Anderson PSS 的参数通常按单台机组本地机电模态整定。
...
在 EMT 中，PSS 控制器需要离散化以便时域求解。常用方法为双线性变换法（Tustin method）：
...
$$x_c(t+\Delta t) = A_c x_c(t) + B_c u(t)$$
...
**方式一：时域截断**（最常用）—— 在每个时步检查 $V_{PSS}$ 是否越限，如越限则直接截断。此方式简单但会在限幅边界引入小幅数值振荡。

**方式二：描述函数法** —— 用描述函数近似限幅环节的谐波传递特性，在小扰动分析中考虑其等效增益下降。

### 与同步机电磁方程的联解

PSS 输出的 $V_{PSS}$ 作为附加输入注入 [[excitation-system]]，AVR 的输出 $E_{fd}$ 再作用于同步机电磁方程。联解链路为：

$$\Delta\omega \xrightarrow{G_W} \xrightarrow{G_C} V_{PSS} \xrightarrow{AVR} E_{fd} \xrightarrow{同步机电磁方程} \Delta T_e \xrightarrow{\text{swing-equation}} \Delta\omega$$
...
多台 PSS 并存时，若参数整定不当，可能出现：相互作用（多台 PSS 在同一模态上产生不利交互）、负阻尼传递（一台 PSS 改善了本地模态但恶化了区域间模态）、控制冲突（PSS 与 FACTS 阻尼控制、HVDC 辅助阻尼控制之间争夺同一频段）。协调整定方法包括协调优化算法和基于广域测量的交互模态分析。

### 挑战三：限幅对大扰动响应的影响

PSS 限幅使小信号设计结论在大扰动下失效。大扰动时：$V_{PSS}$ 可能长时间处于限幅状态，阻尼控制失效；励磁饱和非线性使 $E_{fd}$ 实际值偏离 AVR 指令值；保护动作（AVR 切到手动/备用励磁）使 PSS 退出。需要在 [[transient-stability-analysis]] 框架下对大扰动工况进行验证。
...
- [[wideband-oscillation-stability]] 宽频振荡不能直接用传统 PSS 抑制
- [[electromechanical-electromagnetic-hybrid-simulation]] PSS 在混合仿真中的接口位置
