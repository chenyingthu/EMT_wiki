...
最优潮流（Optimal Power Flow, OPF）是在潮流方程和运行约束下优化电力系统运行点的方法。它把 [[methods/power-flow-calculation.md]] 的等式约束与发电、负荷、电压、线路容量、无功设备和安全约束放入统一优化框架。
...
- 通过 OPF 生成运行方式，再由 [[methods/steady-state-initialization.md]] 转换为 EMT 初始状态；
- 用 OPF 筛选高负荷、弱电压、潮流重载或可再生能源高占比工况；
- 在 [[methods/electromechanical-electromagnetic-hybrid-simulation.md]] 中作为慢时间尺度机电侧或调度侧模型的一部分；
- 对换流器、保护和控制细节，OPF 只能给出设定点或边界条件，不能替代 [[topics/emt-simulation.md]]。
...
- [[sources/damping-of-subsynchronous-control-interactions-in-large-scale-pv-installations-t.md]]：可用于说明优化设定可参与稳定控制策略生成；不能由此推出 OPF 对所有 SSCI 场景有效。
- [[sources/electromechanical-transient-modeling-of-modular-multilevel-converter-based-multi.md]]：包含交直流潮流和动态模型接口，可支撑 OPF/潮流结果作为动态仿真初值的边界说明。
- [[sources/a-novel-interfacing-technique-for-distributed-hybrid-simulations-combining-emt-a.md]]：说明稳态运行点和接口边界会影响混合仿真；不应把优化模型替代 EMT 验证。

## 与相关页面的关系

- [[methods/power-flow-calculation.md]] 是 OPF 的稳态等式约束基础。
- [[methods/economic-dispatch.md]] 可视为不显式建模完整网络约束的简化运行优化问题。
- [[methods/newton-raphson-method.md]] 常用于潮流和部分非线性优化迭代中的方程求解。
- [[methods/steady-state-initialization.md]] 把 OPF 或潮流结果映射到 EMT 初值。
- [[topics/large-scale-power-system.md]] 涉及 OPF 在大规模系统分析中的建模取舍。
