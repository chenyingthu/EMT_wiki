...
```mermaid
graph TD
    subgraph Ncmp[地下电缆建模 (Underground Cable Mo…]
        N0[集总 PI/多段 PI: 把电缆分成 RLCG 段]
        N1[常参数行波模型: 使用固定 $Z_c$ 与传播延时]
        N2[相域频变线路模型: 拟合 $\mathbf{Y}_c$、…]
        N3[节点导纳/FLE 路线: 直接拟合端口导纳或开路/短路块]
        N4[场-路或全波校核: 用场模型计算局部参数/响应]
    end
```
