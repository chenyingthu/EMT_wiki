...
典型输出是注入 AVR 的附加控制信号 $V_{PSS}$。该信号通过励磁系统改变电磁转矩，进而影响 [[swing-equation]] 描述的转子运动。
...
- 基于频率响应的相位补偿；
- 基于特征值和阻尼比的 [[small-signal-stability-analysis]]；
- 基于时域扰动的闭环响应验证；
...
- 在 [[electromechanical-electromagnetic-hybrid-simulation]] 中，PSS 可位于机电侧同步机模型，也可在 EMT 侧详细控制器中实现，取决于分网边界。
...
- [[saturation-in-transient-and-stability-phenomena-for-cylindrical-13&14]]：该 source 在同步机暂态分析中涉及 AVR/PSS 影响，适合支撑 PSS 与励磁/同步机模型耦合的边界说明。
- [[hybrid-simulation-of-power-systems-with-svc-dynamic-phasor-model]]：可作为机电模态和控制装置在混合仿真中处理的相关证据；不能把 SVC 动态相量结论直接外推为 PSS 参数结论。
- [[extending-the-frequency-bandwidth-of-transient-stability-simulation-using-dynami]]：说明机电暂态模型可通过动态相量扩展频带，适合提醒 PSS 所在时间尺度与 EMT 宽频现象不同。

## 与相关页面的关系

- [[excitation-system]] 是 PSS 注入附加控制信号的直接对象。
- [[swing-equation]] 给出阻尼作用最终影响的机电运动方程。
- [[small-signal-stability-analysis]] 是 PSS 参数整定和模态验证的主要分析工具。
- [[transient-stability-analysis]] 关注大扰动下控制器限幅和恢复过程。
- [[wideband-oscillation-stability]] 涉及更宽频段振荡问题，不能直接等同于传统 PSS 低频阻尼。
