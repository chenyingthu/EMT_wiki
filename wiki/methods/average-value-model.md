...
**数值稳定性条件**：
$$
\Delta t \leq \frac{2C_{eq}}{Y_{eq}} \quad \text{或} \quad \Delta t \leq \frac{L_{eq}}{R_{eq}}
$$
...
#### 5.3 验证与校准
- **稳态验证**：与详细模型对比功率流，偏差<2%
- **暂态验证**：故障工况下波形对比，误差<5%
- **频域验证**：传递函数波特图对比
- **能量守恒**：长时间仿真能量误差累计<0.5%

#### 5.4 常见问题与解决

| 问题 | 症状 | 原因 | 解决方案 |
|------|------|------|---------|
| **数值振荡** | 高频纹波不收敛 | 接口延迟/步长过大 | 改用DI-AVM或减小步长 |
| **虚假损耗** | 功率不守恒 | 一步延迟 | 即时联立求解 |
| **直流故障不准** | 电流峰值偏差大 | 未考虑闭锁 | 使用EAVM/UBM-AVM |
| **初始条件漂移** | 启动过程异常 | 初始化不当 | 臂电流初始化补偿 |
| **计算发散** | 仿真中途停止 | 步长超限 | 检查CFL条件 |

#### 5.5 参数敏感性分析
- **子模块电容**：电容越小，AVM误差越大（建议 $C_{SM}$ > 4 mF）
- **开关频率**：频率越高，平均化效果越好（建议 $f_{sw}$ > 1 kHz）
- **调制比**：过调制区域（$m$ > 1）AVM精度下降
- **控制带宽**：控制响应速度应远小于平均化周期
