...
graph TD
    ...
    style Csm fill:#c8e6c9
    style AC_A fill:#f3e5f5
...
- **多类型子模块MMC电磁暂态通用建模和实现方法**
  - 💡 贡献传统单端口MMC建模限制，提出兼容任意多类型/多端口子模块的通用等效建模与拓扑自动识别方法。
...
### 4.3 异构并行加速
- **时间并行算法 (MGRIT)**：贡献空间并行瓶颈，在 5 核并行下实现 $3.47\times$ 加速，适用于含 $1000+$ 子模块的臂内并行。
