...
```mermaid
graph TD
    subgraph Ncmp[换流变压器 (Converter Transformer)]
        N0[**阀侧套管**: 需承受直流电压叠加交流电压，采用直流套管]
        N1[**绝缘设计**: 需考虑直流偏磁和谐波损耗]
        N2[**谐波负载**: 承受大量谐波电流（6脉动：12.5-…]
        N3[**直流偏磁**: 需承受由触发不对称引起的直流偏磁]
        N4[**短路阻抗**: 通常15-18%（较高，限制短路电流）]
        N5[**分接头范围**: ±10%～±20%，级数多]
    end
```
...
换流变压器是高压直流输电系统的核心设备，连接交流电网与换流阀，承担电压变换、电气隔离、短路阻抗和提供换相电抗等功能。与常规电力变压器相比，换流变压器承受谐波负载、直流偏磁和阀侧绝缘等特殊应力。本模型涵盖BCTRAN模型、UMEC模型、饱和特性建模、频变参数建模，适用于HVDC系统设计和暂态分析。
