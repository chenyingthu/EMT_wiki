...
**直流电压下垂控制**：Allabadi (2024) 提出的解耦接口（DI）初始化方法使构网型 VSC 在 MTDC 系统中的初始化迭代减少约 6.9 倍。统一下垂控制策略 (2018) 在 ±500 kV / 4500 MW 四端 MTDC 系统中实现各端电压偏差 < 3% 时的功率自主分配。

**MTDC 潮流计算**：Beerten (2012) 的顺序交直流潮流算法适用于并联型 MTDC 系统，直流网络潮流的牛顿-拉夫逊法迭代通常在 3-5 次内收敛。Liu (2014) 验证了 MMC-MTDC 机电暂态模型在 10 ms 步长下的精度。

**仿真加速比**：张北柔性直流电网示范工程（±500 kV / 4500 MW，四端）的 EMT 仿真中，采用平均值模型的加速比可达详细模型的 10-50 倍，具体取决于换流站详细程度和仿真场景。

**直流故障特性**：MTDC 直流故障电流上升率可达 10-100 kA/s，要求保护装置在 2-5 ms 内动作。混合式直流断路器的开断时间为 2-5 ms，机械式为 20-30 ms。

**数据缺口声明**：不同控制策略（主从、下垂、裕度）在相同 MTDC 测试系统下的暂态性能（电压偏差、恢复时间、功率分配精度）缺乏统一量化基准。高比例新能源接入下 MTDC 的电压稳定边界缺乏系统性的数值评估。
