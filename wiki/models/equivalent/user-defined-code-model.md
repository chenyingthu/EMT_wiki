...
用户自定义代码模型在 EMT 仿真中已有可核验的量化结果，但以下数据均绑定具体接口方式、测试系统和仿真条件，不能外推为通用能力：

- **TACS 教学框架 (2004)** 在 EMTP 中实现了基于 TACS 函数的数字继电器闭环仿真，验证了保护控制逻辑通过 TACS 函数嵌入 EMT 时步循环的可行性。典型采样率为每周波 12-24 点（$f_s = 720-1440$ Hz），全周期 DFT 引入的固有延迟为 **16.67 ms**（1 个 60 Hz 周期），二阶抗混叠低通滤波器可将 DFT 输出幅值误差从 **>15% 降至 <2%**。该框架为教学概念验证，不替代工业级用户自定义代码接口或实时性能测试 (TACS Teaching 2004)。

- **Cai (2018)** 在 EMTP 中实现了基于 FMI 2.0 的 master-slave 协同仿真架构，将控制系统封装为 FMU 从系统与电力网络主系统在内存上解耦。提供异步并行（多个 slave 在同一全局步长上并行运行）和同步多步（slave 在顺序环境中执行多个内部子步）两种模式，支持控制子系统采用不同内部步长。文章称在大规模保护系统研究中观察到可观的计算时间收益并保持精度，但未报告可核验的加速比或误差上限 (Cai 2018)。

- **Chaudhary (2004)** 在 EPRI/DCG EMTP Version 2.0 中集成了 FORTRAN 用户子程序接口，保护控制算法通过该接口接入 EMT 闭环仿真。跳闸信号通过 FORTRAN 子程序反馈到网络状态，实现逐时步闭环。原文未报告可核验的动作时间或误差指标 (Chaudhary 2004)。
...
- FMI 2.0 控制解耦方法报告了计算时间收益但未给出可核验的加速比和误差上限（Cai 2018），不同耦合强度下的接口误差和稳定性边界尚不明确。
- TACS/用户自定义代码的采样率选择（720-1440 Hz）在更高频电力电子接口场景下的适应性缺乏系统评估。
- FORTRAN 用户子程序接口（Chaudhary 2004）在现代 EMT 平台中的兼容性和与现代控制系统建模语言的桥接缺乏标准化更新。
