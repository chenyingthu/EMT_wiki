...
恒功率负载模型在 EMT 仿真中已有可核验的量化结果，但以下数据均绑定具体建模方法和验证条件，不能外推为通用能力：

- **Alboaouh (2025)** 提出了一种逐时间步求解的感性恒功率负载 EMT 模型，通过引入虚拟变量（dummy variables）将 RMS 计算从传统每周期 O(N) 批量求和转换为每步 O(1) 递归更新，消除了 Part I 模型需整周期批量计算的约束，使模型可直接嵌入 EMT 逐时间步求解器。该模型严格维持固定有功功率和固定功率因数约束，功率因数偏差在仿真过程中保持为零，适用于正弦和非正弦工况。验证基于恒阻抗负载合成数据对比，结论表述为 satisfactory，原文未报告可核验的数值误差百分比或仿真加速比（Alboaouh 2025）。

- **Milani (2021)** 在 EMTP 中实现了详细元件级综合负荷模型（涵盖 SMPS、LED 驱动、感应电机和变频驱动），并用 Hydro-Quebec 现场录波数据验证。商业案例中，1.52 MW 负荷在 0.296 s 暂降期间三相电压最大跌落 67.9%，仿真电流谐波频谱与实测吻合。电机接触器脱扣阈值精确位于额定电压 45%~55% 区间且需持续 1~3 个工频周波；电压恢复至约 65% 额定值后，70% 的电机负荷在 20 ms 内完成重合闸。居民案例中，7 MW 预故障负荷因电压暂降损失 1.5 MW 工业电机负荷（约 21.4%）。该模型不直接等价于理想 CPL，其证据边界限于 EMTP 离线仿真和 Hydro-Quebec 特定馈线录波数据（Milani 2021）。

这些量化数据不构成对 CPL 建模方法的全面性能评价，只说明在特定测试条件下可获得的能力边界。
