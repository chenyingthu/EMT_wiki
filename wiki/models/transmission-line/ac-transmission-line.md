...
交流输电线路模型在 EMT 仿真中的性能已有可核验的量化结果，但以下数据均绑定具体线路模型、硬件平台和验证条件，不能外推为通用能力：

- **Chen & Dinavahi (2009)** 在单片 FPGA 上实现了含 15 条全频变输电线路的实时 EMTP 仿真。采用两级并行（功能模块级并行 + 模块内部流水线）和 46 位自定义浮点格式，在 **12.5 ns** FPGA 时钟周期下实现 **12 μs** 实时仿真步长。实时示波器捕获波形与 ATP 离线仿真对比，关键节点电压/电流峰值误差 **<0.5%**，暂态振荡相位偏差 **<0.2°**。该方法限于单片 FPGA 资源和 15 条线路的测试规模，不自动适用于更大规模电网或含强非线性电力电子装置的系统（Chen & Dinavahi 2009）。

- **Liu (2021)** 在 Xilinx FPGA（VC707/VCU118）上实现了频变相域（FDPD）输电线路模型的实时仿真，仿真步长压缩至 **2.4 μs~3.27 μs**，较传统处理器模型提升约 **15~20 倍**（详见 [[frequency-dependent-line-model]]）。

- **Colqui (2022)** 在 ATP-EMTP 中实现了基于理想变压器阵列的 Clarke 模态变换电路，用于构建模态域输电线路模型。该方法将三相线路解耦为 α、β、0 三个独立模态后连接 Bergeron 或 JMarti 模型，验证对象为完全换位三相线路。该方法不改变线路模型的数值精度，主要贡献在于突破商用软件内置模型限制，允许用户自定义频变参数（Colqui 2022）。

这些量化数据不构成对交流输电线路建模方法的全面性能评价，只说明在特定测试条件下可获得的能力边界。
