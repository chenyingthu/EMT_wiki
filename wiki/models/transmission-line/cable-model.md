...
- **A new tool for calculation of line and cable parameters**
  ...
  - 提出基于MoM-SO理论的新型参数计算工具，贡献传统EMT软件内置程序的局限。
...
传统基于几何参数的模型在高频段（>500 kHz）误差显著。**最新趋势**是将实测同轴传播特性（衰减常数 $\alpha$ 和波速 $v$）通过贝叶斯推断或机器学习融合进解析模型，解决绞线效应和半导电层损耗的建模难题。关键贡献点在于保持低频特性不变的同时修正高频阻尼。
