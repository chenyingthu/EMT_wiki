...
- 模型提取计算耗时仅约 **1分钟**（CPU时间），满足工程快速建模需求
...
- 大型电网基准系统（含202台变压器、58条线路、72台同步机）中，77极点FDNE覆盖 **1 Hz至1 kHz**，计算效率提升 **2.16倍**（10秒仿真：26秒→12秒）
- 12核并行架构实现 **14.4倍** 加速（含15,136个控制模块及非线性开关器件）
- 混合协同仿真中EMT子步长可设置为PDT步长的1/5
- 数据缺口：该文为综述性特写，FDNE加速数据来自大型电网而非HVDC换流变压器专项验证
