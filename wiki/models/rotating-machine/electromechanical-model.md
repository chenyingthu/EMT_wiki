...
机电暂态模型在混合仿真中的性能已有可核验的量化结果，但以下数据均绑定具体测试系统和验证条件，不能外推为通用能力：

- **Sun (2014)** 针对并网光伏发电系统提出了通用机电暂态模型，在 PSCAD/EMTDC 中与详细电磁暂态模型（含 DC/DC 升压、PWM 逆变器和 PQ 解耦控制）对比。电磁暂态模型因 **2000 Hz** 开关频率采用 **50 μs** 步长，机电暂态模型采用 **1 ms** 步长（步长扩大 20 倍）。在 **10 s** 仿真时长下，计算时间从 **46.6 s** 降至 **4.1 s**，效率提升约 **11.4 倍**。验证场景限于光照强度阶跃扰动，未覆盖电网故障、低电压穿越或弱电网工况（Sun 2014）。

- **Chen (2022)** 在 ADPSS 平台上实现了含大量电磁直流模型的机电-电磁混合仿真自动化框架。混合仿真单运行方式建模时间从 **15 h** 压缩至 **0.5 h**（约 **30 倍**），15 个方式批量校核节省超 **200 h** 人工工作量。电磁直流模型初始化误差：直流功率与电压偏差 **<1%**，触发角/关断角偏差 **<1°**，无功交换偏差小于半组滤波器容量。接口箝位启动策略使混合仿真在 **0.6 s** 内达到稳态。该结论基于 ADPSS 平台和 10 余回特高压/高压直流的实际电网验证，不自动适用于 MMC-HVDC 或新能源控制深度耦合场景（Chen 2022）。

这些量化数据不构成对机电暂态建模方法的全面性能评价，只说明在特定测试条件下可获得的能力边界。
