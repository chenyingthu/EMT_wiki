...
### 5.1 建模前准备
1. **确定研究目标**：阀级机理研究选详细模型，系统级控制验证选PAVM/TBSD
2. **交流系统强度评估**：SCR（短路比）<2时换相失败风险显著增加，需精细建模
3. **控制保护策略**：明确是否含低压限流(VDCOL)、熄弧角调节(γ-control)等
...
| **关断角裕度** | γ ≥ 9° | 安全运行边界 |
| **触发角范围** | 5°-25° | 正常稳态运行 |
| **换相电抗** | 0.1-0.2 p.u. | 典型值，影响换相失败概率 |
| **直流电抗** | 0.3-0.5 H | 抑制直流电流纹波 |

### 5.3 故障场景设置
- **单相接地故障**：最常见换相失败诱因，需设置故障阻抗0-100Ω
- **三相短路**：极端工况，验证模型稳定性
- **直流线路故障**：测试保护配合与恢复策略

### 5.4 验证与校核清单
- [ ] 稳态波形与解析解对比（电压电流幅值、相位）
- [ ] 换相失败时刻捕捉精度（与详细模型对比）
- [ ] 直流故障恢复过程验证
- [ ] 功率反转动态响应测试
- [ ] 与交流系统强/弱连接工况验证
...
| 问题 | 症状 | 原因 | 解决方案 |
|------|------|------|---------|
| **换相失败预测不准** | 波形与详细模型差异大 | 关断角计算简化 | 改用TBSD或引入修正系数 |
| **仿真发散** | 直流电压电流振荡增大 | 数值稳定性不足 | 减小步长或改用L稳定积分方法 |
| **计算太慢** | 大规模系统无法实时 | 矩阵维度过高 | 采用PAVM或多速率MATE |
| **不平衡工况偏差** | 负序电流计算不准 | 谐波简化过度 | 启用正负序分离扩展 |
| **实时性不足** | 步长<计算耗时 | 模型复杂度过高 | FPGA加速或模型降阶 |
...
### 案例1：常规HVDC换相失败机理研究
**场景**：某±800kV/8000MW特高压直流输电系统，研究交流系统故障导致的换相失败
**方案**：
- 模型选择：TBSD详细模型（步长20μs）
- 故障设置：送端单相接地，接地阻抗50Ω
- 对比验证：与RTDS实时仿真结果对比
**结果**：
- 换相失败时刻预测误差<0.1ms
...
### 案例2：多馈入HVDC系统相互作用分析
**场景**：含3回LCC-HVDC的多馈入系统，分析换相失败在多回直流间的传播
**方案**：
- 模型选择：扩展PAVM（支持不平衡工况）
- 电气距离：各回直流间电气距离20-100km
- 仿真规模：交流系统1000节点，3回12脉动换流器
**关键发现**：
- 第一回换相失败后，相邻直流换相失败概率增加35%
- 虚拟阻抗控制可使连续换相失败次数减少40%

### 案例3：混合HVDC（LCC-MMC）实时仿真
**场景**：LCC与MMC级联的混合HVDC系统，用于海上风电送出
**方案**：
- 拓扑：LCC整流站 + MMC逆变站
- 模型：多速率MATE（LCC侧步长50μs，MMC侧步长10μs）
- 硬件：CPU-FPGA协同，FPGA实现阀级模型
**性能**：
- 实时性：实际计算时间/仿真时间 = 0.85
- 精度：NRMSE<5%
- 接口延迟：<1μs
