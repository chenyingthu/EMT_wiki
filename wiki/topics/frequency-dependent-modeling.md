...
title: "频率相关建模"
type: topic
tags: ["EMT仿真", "矢量拟合", "频变输电线路", "FDNE", "宽频建模"]
created: "2026-04-13"
---
...
电力系统元件的电磁参数具有显著的频率依赖性，主要体现在：

1. **导体集肤效应(Skin Effect)**：交流电阻随频率升高而增大，内电感减小
   $$R(f) = R_{dc} \cdot k_r(f), \quad L_i(f) = L_{i,dc} \cdot k_l(f)$$

2. **大地回路阻抗**：Carson公式描述的大地电阻和电感随频率变化
   $$Z_g(f) = \pi^2 f \cdot 10^{-4} + j4\pi f \cdot 10^{-4} \ln\left(\frac{D_e}{GMR}\right)$$
   其中等效深度 $D_e = 1.85/\sqrt{\mu_0 \sigma f}$

3. **介质损耗**：电缆绝缘和地介质的复介电常数频变特性
   $$\varepsilon(\omega) = \varepsilon'(\omega) - j\varepsilon''(\omega) = \varepsilon_\infty + \frac{\varepsilon_s - \varepsilon_\infty}{1+(j\omega\tau)^\alpha}$$

### 1.2 矢量拟合(Vector Fitting, VF)技术

矢量拟合是频变建模的核心算法，由Gustavsen和Semlyen于1999年提出，用于将频域响应 $H(s)$ 拟合为有理函数形式：

$$H(s) \approx \sum_{k=1}^{N} \frac{r_k}{s-p_k} + d + s \cdot e$$

其中 $p_k$ 为极点，$r_k$ 为留数，$d$ 为常数项，$e$ 为微分项。

**算法步骤**：

1. **极点重定位**：通过引入辅助函数 $\sigma(s)$ 将非线性问题转化为线性最小二乘问题
   $$\sigma(s) = \sum_{k=1}^{N} \frac{\tilde{r}_k}{s-\tilde{p}_k} + 1$$
   $$\sigma(s)H(s) \approx \sum_{k=1}^{N} \frac{r_k}{s-\tilde{p}_k} + d + s \cdot e$$

2. **线性求解**：在采样频率点 $s_m = j\omega_m$ 构建超定方程组
   $$A \cdot x = b$$
   其中 $x = [\tilde{r}_1, \dots, \tilde{r}_N, r_1, \dots, r_N, d, e]^T$

3. **极点提取**：通过 $\sigma(s)$ 的零点获得新的极点 $p_k$（即 $\tilde{p}_k - \tilde{r}_k$ 的 eigenvalues）

4. **留数计算**：固定极点后，重新求解线性问题获得最优留数

**改进算法**：
- **松弛矢量拟合(Relaxed VF)**：改善高频拟合精度
- **快速残差计算**：利用C语言底层线性代数库（BLAS/LAPACK）加速矩阵运算，计算性能提升可达 $2-5\times$

### 1.3 相域与模域建模策略

**模域方法(Modal Domain)**：
- 通过常数变换矩阵 $T$ 将相量转换为模量：$I_m = T^{-1}I_{ph}$
- 假设变换矩阵在宽频范围内恒定（基于工频或几何平均频率计算）
- 计算效率高，但在高频（>10kHz）或严重非对称线路中精度下降

**相域方法(Phase Domain)**：
- 直接在相坐标下建模，避免模态转换误差
- 通用线路模型(ULM)采用相域直接拟合：
  $$Y_c(s) = \sum_{k=1}^{N} \frac{R_k}{s-p_k} + D$$
  $$H(s) = \sum_{k=1}^{N} \frac{R_k}{s-p_k} + D \cdot e^{-s\tau}$$
- 精度高但计算复杂度为 $O(n^3)$，$n$ 为导体数

**频变相域(FDPD)混合方法**：
结合两者优势，对地模采用精确频变模型，对线模采用简化模型，在保证精度的同时降低 $30-40\%$ 计算负担。

### 1.4 频率相关网络等值(FDNE)

FDNE用于将外部大网络等值为多端口频变诺顿等效：

$$I_{eq}(s) = Y_{eq}(s)V(s) + I_{hist}(s)$$

其中等值导纳矩阵 $Y_{eq}(s)$ 通过端口测量或网络约简获得，并采用矢量拟合转换为时域递归卷积形式：

$$i(t) = G_0 v(t) + \sum_{k=1}^{N} i_k(t)$$
$$\frac{di_k}{dt} = p_k i_k(t) + r_k v(t)$$

**8端口FDNE**：适用于变电站母线等值，可精确保持 $100\text{Hz}-10\text{kHz}$ 范围内的谐波阻抗特性。

### 1.5 递归卷积与状态空间实现

将频变导纳转换为时域离散伴随模型：

$$i(t) = G_{eq}v(t) + I_{hist}(t-\Delta t)$$

历史电流源更新采用递归公式避免存储完整卷积历史：

$$I_{hist}(t) = \sum_{k=1}^{N} \alpha_k I_{hist,k}(t-\Delta t) + \beta_k [v(t) + v(t-\Delta t)]$$

其中系数 $\alpha_k = e^{p_k \Delta t}$，$\beta_k = \frac{r_k}{p_k}(1-\alpha_k)$。

**数值稳定性**：极点 $p_k$ 必须位于左半平面（负实部），对于电缆的介质损耗模型，需采用**修正矢量拟合**确保无源性和稳定性。

## 2. 硬件平台对比

基于论文中提及的FPGA实现与并行计算技术，不同硬件平台的频变建模性能对比如下：

| 平台 | 架构特点 | 矢量拟合性能 | 递归卷积性能 | 适用场景 | 局限性 |
|------|----------|--------------|--------------|----------|--------|
| **CPU (x86)** | 串行执行，Cache层次丰富 | 中等（依赖BLAS库） | 高灵活性，适合复杂逻辑 | 离线仿真、算法验证 | 实时性难以保证（大规模网络） |
| **FPGA** | 流水线并行，确定时延 | 预计算阶段离线，在线查表 | 极高并行度，固定步长 | 实时仿真、HIL测试 | 开发周期长，浮点资源受限 |
| **GPU (CUDA)** | 大规模SIMD并行 | 批量拟合加速 $10-50\times$ | 历史电流并行更新 | 蒙特卡洛分析、参数扫描 | 数据传输开销大，小模型效率低 |
| **混合架构** | CPU+FPGA协同 | CPU预处理，FPGA执行 | FPGA处理计算密集型部分 | 大规模电网实时仿真 | 接口同步复杂 |

**FPGA优化策略**：
- 极点-留数对采用定点数或低精度浮点（半精度）存储
- 递归卷积实现为流水线结构，每个时钟周期更新一个历史电流源
- 对于ULM模型，采用**双端口RAM**存储历史电压，**ROM**存储指数衰减系数 $\alpha_k$

**并行计算技术**：
- 多导体线路的相域导纳矩阵计算可并行化（OpenMP）
- 多时间尺度仿真中，频变部分采用小步长 $\Delta t_{EMT}$，外部网络采用大步长 $\Delta t_{TS}$

## 3. 实际应用案例汇总

### 3.1 高压电缆系统宽频建模

**应用场景**：500kV XLPE电缆护层暂态过电压分析
- **模型**：频变电缆模型(FDCM) + FDNE等值外部网络
- **关键参数**：
  - 导体集肤效应：采用9阶有理函数拟合（VF）
  - 绝缘介质：Debye模型描述 $\varepsilon_r(f)$
  - 护层接地方式：交叉互联，考虑护层环流频变阻抗
- **结果**：相比常数参数模型，高频过电压峰值误差从 $18\%$ 降至 $3\%$ 以内

### 3.2 非平行架空线路暂态分析

**应用场景**：输电走廊空间受限导致的非平行多回线路（基于2022年DSFTL模型）

**技术细节**：
- 考虑线路交叉角 $\alpha$（$0^\circ-90^\circ$）的互感频变耦合
- 大地损耗采用复镜像深度近似：
  $$p \approx \frac{1}{\sqrt{j\omega\mu_0\sigma_g}}$$
- 散射场积分闭式解在 $f < 100\text{kHz}$ 有效

**性能指标**：
| 指标 | 全波仿真(FDTD) | DSFTL频变模型 | 提升倍数 |
|------|----------------|---------------|----------|
| 计算时间 | 数小时 | $<10\text{s}$ | $>1000\times$ |
| 内存占用 | GB级 | MB级 | $>100\times$ |
| 峰值电压误差 | 基准 | $<3\%$ | - |
| 波前时间误差 | 基准 | $<5\%$ | - |

### 3.3 并网变换器宽频稳定性分析

**应用场景**：新能源场站接入弱电网的次/超同步振荡
- **方法**：基于测量数据的黑箱FDNE建模
- **频段**：$10\text{Hz}-2\text{kHz}$（关注次同步谐振SSR）
- **验证**：与现场实测阻抗频率扫描数据对比，幅值误差 $<2\text{dB}$，相位误差 $<5^\circ$

### 3.4 耦合电容式电压互感器(CCVT)暂态建模

**应用场景**：1998年提出的详细CCVT模型用于继电保护配合研究
- **频变元件**：分压电容 $C_1, C_2$ 的高频特性
- **非线性**：中间变压器饱和特性（磁滞回线）
- **谐振抑制**：阻尼电路的频变电阻 $R(f)$
...
针对含电力电子设备的电网，需同时仿真：
- **低频段**（机电暂态，$<10\text{Hz}$）：简化线路模型，重点在控制系统
- **中频段**（谐波，$10\text{Hz}-2\text{kHz}$）：频变线路模型 + 平均化变换器模型
- **高频段**（电磁暂态，$>2\text{kHz}$）：详细开关模型 + 宽频线路模型

**动态相量法(Dynamic Phasors)**与EMT混合：通过解析信号理论，在不同频段自动切换模型复杂度，计算效率提升可达 $5-10\times$。

### 4.2 数据驱动的频变建模

**黑箱建模挑战**：
- 基于端口测量的FDNE难以保证无源性(Passivity)
- 需开发**保形有理拟合(Shape-Preserving Rational Fitting)**，确保 $\text{Re}\{Y(s)\} > 0$ 对所有 $s$ 成立

**AI辅助方法**：
- 利用神经网络替代传统VF，加速多工况批量拟合
- 物理信息神经网络(PINN)融合麦克斯韦方程约束，提升外推能力

### 4.3 实时仿真中的计算瓶颈

**开放问题**：
1. **大规模网络**：当FDNE端口数 $>50$ 时，状态空间实现导致矩阵维度爆炸，需发展**稀疏实现**或**模型降阶(MOR)**技术
2. **变步长算法**：频变模型通常要求固定小步长 $\Delta t \leq \Delta z/c$，自适应步长算法与递归卷积的兼容性
3. **多导体电缆**：三芯电缆的频变屏蔽层耦合模型在相域中计算量巨大，需开发专用对称性简化算法

### 4.4 新型电力系统宽频振荡

**宽频带稳定性分析**：
- 新能源并网引入的宽频振荡（$10\text{Hz}-1\text{kHz}$）需要精确的频变网络阻抗模型
- 现有EMT工具在 $>1\text{kHz}$ 频段缺乏标准频变设备模型（如GIS、SVG）
- **需求**：开发覆盖 $0-10\text{kHz}$ 的统一频变元件库，支持谐波域与相域联合仿真

### 4.5 非平行与复杂几何线路

**趋势**：随着城市电网密集化，线路非平行、交叉、同塔多回等情况增多
- 传统假设（无限长平行线）引入的系统误差可达 $10-15\%$
- 需结合**全波电磁仿真**(FDTD/FEM)提取等值参数，再通过FDNE嵌入EMT仿真
- 计算效率与精度的平衡仍是核心挑战（参见2022年DSFTL工作）