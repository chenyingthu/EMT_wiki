...
```mermaid
graph TD
    subgraph Ncmp[继电保护 (Relay Protection)]
        N0[时间配合: 靠近故障点的保护先动作]
        N1[电流配合: 故障电流大的保护先动作]
        N2[阻抗配合: 距离故障近的保护先动作]
        N3[方向配合: 判别故障方向]
...
$$I_{rms} > I_{pickup} \quad \text{且} \quad t > t_{delay}$$

反时限特性：
$$t = \frac{TMS \cdot k}{(I/I_{pickup})^\alpha - 1}$$

其中 TMS 为时间倍数设定，$k$ 和 $\alpha$ 为曲线常数（IEC标准：k=0.14, α=0.02 标准反时限）。

### 距离保护测量阻抗

相间距离：
$$Z_{\phi\phi} = \frac{V_{\phi\phi}}{I_{\phi\phi}}$$

接地距离（零序补偿）：
$$Z_{\phi g} = \frac{V_{\phi}}{I_{\phi} + k_0 I_0}$$
...
基本差流：
$$I_{diff} = |\sum_{j=1}^{n} I_j|$$

制动电流：
$$I_{rest} = \frac{1}{2}\sum_{j=1}^{n} |I_j|$$

动作条件：
$$I_{diff} > k \cdot I_{rest} + I_{min}$$

其中 $k$ 为制动系数（通常0.2~0.5），$I_{min}$ 为最小动作电流。
...
通常 $k_{2nd} = 0.15 \sim 0.20$。

五次谐波制动：
$$\frac{I_5}{I_1} > k_{5th}$$

通常 $k_{5th} = 0.35 \sim 0.40$。
...
| 配合类型 | 原则 | 实现方法 |
|----------|------|----------|
| 时间配合 | 靠近故障点的保护先动作 | 逐级增加延时（0.3~0.5s） |
...
| 过流延时 | 0.3~2s | 后备保护 |
| 距离I段 | 0~0.1s | 线路80%~85% |
| 距离II段 | 0.3~0.5s | 全线+相邻线路 |
| 差动保护 | 0.02~0.05s | 变压器、母线 |
| 行波保护 | 0.001~0.01s | 超高速保护 |

### 保护误动/拒动原因分析

| 现象 | 可能原因 | 对策 |
|------|----------|------|
| 误动 | 涌流、CT饱和 | 谐波制动、比率制动 |
