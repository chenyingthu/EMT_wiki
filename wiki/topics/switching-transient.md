...
```mermaid
graph TD
    subgraph Ncmp[开关暂态 (Switching Transient)]
        N0[空载线路合闸: 2.0~3.0]
        N1[电容器组投切: 2.0~2.5]
        N2[并联电抗器开断: 3.0~4.5]
        N3[变压器合闸: 1.5~2.0]
...
其中 $V_m$ 为电源电压峰值，$Z_0$ 为线路波阻抗，$Z_s$ 为系统阻抗，$k_{af}$ 为振幅系数（考虑残余电荷和相位）。

考虑合闸相位角 $\phi$ 和残余电荷 $V_{res}$：
$$V_{max} = V_m\sin\phi - V_{res}$$

统计过电压（$2\%$ 概率）：
$$V_{2\%} = k_a \cdot k_{st} \cdot V_{max}$$

其中 $k_a$ 为考虑三相不同期系数（$1.2\sim1.4$），$k_{st}$ 为统计系数。

### 分闸暂态恢复电压

恢复电压上升率(RRRV)：
$$\text{RRRV} = \frac{dv}{dt}\bigg|_{t=t_0} = \omega I_0 \sqrt{\frac{L}{C}}$$

其中 $I_0$ 为开断电流，$L$ 为电感，$C$ 为等效电容。
...
其中 $I_{chop}$ 为截流电流，$L$ 为负载电感，$C$ 为杂散电容。
